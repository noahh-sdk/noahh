...
ask_gd_path() {
    ...
    while read -p "Enter the path where GeometryDash.exe is located: " POTENTIAL_PATH < /dev/tty; do
        ...
        if is_valid_gd_path "$POTENTIAL_PATH"; then
            if confirm "Do you want to install ${YELLOW}Noahh${NC} to ${YELLOW}$POTENTIAL_PATH${NC}?"; then
...
install() {
    ...
    if [ -z "$TAG" ]; then
        echo -e "${RED}Error:${NC} Noahh tag is not set." >&2
        exit 1
    fi
    ...
    echo ""
    echo "Downloading Noahh $TAG..."
    if ! curl -L -o "$TEMP_DIR/noahh.zip" "https://github.com/noahh-sdk/noahh/releases/download/$TAG/noahh-$TAG-win.zip"; then
        echo -e "${RED}Error:${NC} Failed to download Noahh." >&2
        exit 1
    fi

    echo "Unzipping..."

    if ! unzip -qq "$TEMP_DIR/noahh.zip" -d "$TEMP_DIR/noahh"; then
        echo -e "${RED}Error:${NC} Failed to unzip Noahh." >&2
        exit 1
    fi

    echo "Installing..."
    mv $TEMP_DIR/noahh/* "$GD_PATH"
}
 ...
 ..@@@@@.@@@@@@@@@@@..@@@@@.. 
   ..@@@..............@@@..   
    ..@@@@@@@@@@@@@@@@@@..    
      ...@@@@@@@@@@@@...      
EOF
echo "Installing Noahh..."

# Get latest tag from the Index
VERSION_JSON="$(curl -s 'https://api.noahh-sdk.org/v1/loader/versions/latest?platform=win')"
if [ -x "$(command -v jq)" ]; then
...
# If Noahh Index fails, try GitHub API
if [ -z "$TAG" ]; then
    echo "Noahh Index failed, checking GitHub releases..."
    GITHUB_JSON="$(curl -s 'https://api.github.com/repos/noahh-sdk/noahh/releases?per_page=1')"
    if [ -x "$(command -v jq)" ]; then
...
if [ -z "$TAG" ] || [ "$TAG" = "null" ]; then
    echo -e "${RED}Error:${NC} Failed to get latest version from Noahh Index or GitHub."
    exit 1
fi

echo -e "Detected latest ${YELLOW}Noahh${NC} version as ${BLUE}$TAG${NC}."
...
if ! find_gd_installation; then
    ...
    if ! confirm "Do you want to install ${YELLOW}Noahh${NC} to ${YELLOW}$GD_PATH${NC}?"; then
        ask_gd_path
    fi
fi
...
echo -e "Noahh has been installed successfully at ${YELLOW}$GD_PATH${NC}!"
echo -e "┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓"
