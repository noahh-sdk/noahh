...
; settings
    Name "Noahh"
    OutFile "noahh-installer-win.exe"
    Unicode true
...
; languages
    ...
    !insertmacro NOAHH_LANGUAGE "English"
    !insertmacro NOAHH_LANGUAGE "French"
    !insertmacro NOAHH_LANGUAGE "German"
    !insertmacro NOAHH_LANGUAGE "Spanish"
    !insertmacro NOAHH_LANGUAGE "SpanishInternational"
    !insertmacro NOAHH_LANGUAGE "Italian"
    !insertmacro NOAHH_LANGUAGE "Swedish"
    !insertmacro NOAHH_LANGUAGE "Greek"
    !insertmacro NOAHH_LANGUAGE "Finnish"
    !insertmacro NOAHH_LANGUAGE "Polish"
    !insertmacro NOAHH_LANGUAGE "Russian"
    !insertmacro NOAHH_LANGUAGE "PortugueseBR"
    !insertmacro NOAHH_LANGUAGE "Portuguese"
    !insertmacro NOAHH_LANGUAGE "Ukrainian"
    !insertmacro NOAHH_LANGUAGE "Czech"
    !insertmacro NOAHH_LANGUAGE "Turkish"
    !insertmacro NOAHH_LANGUAGE "Japanese"
    !insertmacro NOAHH_LANGUAGE "SimpChinese"
    !insertmacro NOAHH_LANGUAGE "TradChinese"
    !insertmacro NOAHH_LANGUAGE "Korean"
...
Var noahh.DirectoryPage.ErrorText
...
Function DirectoryPageShow
    ...
    Pop $noahh.DirectoryPage.ErrorText
    ShowWindow $noahh.DirectoryPage.ErrorText 0
    SendMessage $mui.DirectoryPage ${WM_GETFONT} 0 0 $0
    SendMessage $noahh.DirectoryPage.ErrorText ${WM_SETFONT} $0 1
    SetCtlColors $noahh.DirectoryPage.ErrorText ff0000 transparent
    ; place the label at the top
    System::Call 'USER32::SetWindowPos(p$noahh.DirectoryPage.ErrorText, p0, i0, i0, i0, i0, i3)i'
    Pop $0
FunctionEnd
...
Function .onVerifyInstDir
    ...
    ; check if noahh is already installed
    IfFileExists $INSTDIR\Noahh.dll valid
    ...
    noGameNoLife:
        SendMessage $noahh.DirectoryPage.ErrorText ${WM_SETTEXT} "" "STR:$(NOAHH_TEXT_GD_MISSING)"
        Goto error
    versionIssueImo:
        SendMessage $noahh.DirectoryPage.ErrorText ${WM_SETTEXT} "" "STR:$(NOAHH_TEXT_GD_OLD)"
        Goto error
    other_hackpro:
    ...
    other_xinput:
        ...
        Goto other
    other:
        ${StrRep} $0 $(NOAHH_TEXT_MOD_LOADER_ALREADY_INSTALLED) "the dll trademark" $0
        SendMessage $noahh.DirectoryPage.ErrorText ${WM_SETTEXT} "" "STR:$0"
        SetCtlColors $noahh.DirectoryPage.ErrorText e25402 transparent
        LockWindow off
        Return

    error:
        ShowWindow $noahh.DirectoryPage.ErrorText 1
        SetCtlColors $noahh.DirectoryPage.ErrorText ff0000 transparent
        LockWindow off
        Abort
        Return
...
SectionGroup "Noahh"
    Section "Loader" LOADER_SECTION
        ...
        File ${BINDIR}\Noahh.dll
        File ${BINDIR}\Noahh.pdb
        File ${BINDIR}\NoahhUpdater.exe
        ...
        WriteUninstaller "NoahhUninstaller.exe"
    SectionEnd
...
Function un.onInit
    ...
    ; check if xinput and noahh exist
    IfFileExists $INSTDIR\XInput1_4.dll 0 invalid
    IfFileExists $INSTDIR\Noahh.dll 0 invalid
        Return

    invalid:
        MessageBox MB_ICONSTOP|MB_OK $(NOAHH_UNTEXT_NOAHH_MISSING)
        Abort
FunctionEnd
Section "Uninstall"
    DeleteRegKey /ifempty HKCU "Software\Noahh"
    Delete $INSTDIR\NoahhUninstaller.exe
    Delete $INSTDIR\Noahh.dll
    Delete $INSTDIR\Noahh.pdb
    Delete $INSTDIR\Noahh.lib
    Delete $INSTDIR\NoahhUpdater.exe
    Delete $INSTDIR\XInput1_4.dll

    # default value of DATA is an empty string
    # if DATA is empty, keep user data
    # otherwise, delete the entire noahh and DATA\noahh\mods dirs
    # the reason we're deleting DATA\noahh\mods instead of just passing
    ...
    remove_data:
        ...
        RMdir /r $0\noahh\mods ; delete DATA\noahh\mods
        RMdir $0\noahh ; then delete DATA\noahh non-recursively, assuming mods is the only directory in DATA\noahh
        Return

    invalid:
        # this message doesnt rly need translatable as
        # its only supposed to be used internally by noahh itself
