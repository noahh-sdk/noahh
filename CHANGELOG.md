 ...
 * Fix modifying values on `noahh::Button` when held (#1818)

## v5.0.0-beta.4
 * Add `requires-patching` mod.json key for iOS (3b5e715)
 * Add `noahh::Button` (#1810)
 ...
 * Move some buttons to noahh settings (403b546)
 ...
 * Add `noahh::utils::getInputTimestamp` (#1751)
 ...
 * Add restart button to Noahh menu (#1739)
 * Add event for logs, expose more fields (7ccd613)
 * Remove deprecated `noahh::cocos::getChild` (4120659)
 ...
 * Add `noahh::NineSlice`, a better alternative to `CCScale9Sprite` (#1698)
 * Add SceneEvent (#1696)
 ...
 * Include the `noahh::async` namespace in noahh prelude (7ef3881)
 ...
 * Remove `noahh::cast:as`
 * Remove `CCARRAY_FOREACH` and `CCDICT_FOREACH` macros
   * Use `CCArrayExt`, `CCDictionaryExt` instead
 * Replace most `std::function` with `noahh::Function` and `noahh::FunctionRef`
 * `noahh::Popup` is no longer templated (dfe5f74)
 ...
 * Use [dankmeme01/arc](https://github.com/dankmeme01/arc) as new async runtime, ~~mostly~~ replacing `noahh::Task`
 ...
 * Add `modify_cast` into `noahh::cast` (#1485)
 * Add "clean mode", allowing Noahh to not be loaded by holding Alt+Shift during launch (#1669)
...
## ~~v4.11.0~~
 * Add random utils in `noahh::utils::random` (6da879b, 5abd3a9, ad2146d)
 ...
 * Allow changing library link type in `setup_noahh_mod` (9fdc5b5)
 * Allow using `string_view` and `const char*` as `CCDictionaryExt` key (c14d02c)
 * Rename async.hpp to coro.hpp, and noahh::utils::coro namespace (7a2dc8d, 3b94915)
 * Remove `CoTask`, add `coro::spawner` and other utilities (671d321)
 * Fix `noahh::stl::vector` on r29 ndk (#1508)
 ...
 * Add `noahh::cocos::getObjectName` for getting demangled type names (c734b29)
 * Add mod load priority, load mods in alphabetical order (#1492, #1500)
 * Implement `CCImage::saveToFile` for macOS and iOS (#1496)
 * Allow using SDK without `NOAHH_MOD_ID` macro (useful for static libs) (a6f215f)
...
## v4.8.0
 * Fix `noahh::Notification` z ordering (127733e)
 * Add `CCNode::getChildrenExt` (d198df5)
 * Add `CCNode::getChildByIndex` (0ba1f2a)
 * Fix `NOAHH_MOD_STATIC_PATCH` not using the noahh namespace (#1456)
 ...
 * Deprecate the usage of noahh::cast::as (9d28ab9)
 ...
 * Unzip JIT-less iOS binaries separately from the .noahh file (85fe11f)
 ...
 * JIT-less iOS support! (#1395)
   * This means that Noahh now supports iOS 26 beta and above
   * Follow instructions in the [iOS launcher guide](https://github.com/noahh-sdk/ios-launcher/blob/main/INSTALL.md) for setting up
   ...
   * There are new additions to help with JIT-less specific hooking and patching
     * Noahh's own `Modify` classes should work out of the box
     * You can check if the platform is JIT-less iOS by using the `Loader::isPatchless` function
     * For custom hooks, you can use the `NOAHH_MOD_STATIC_HOOK` macro to add a hook. This hook can be enabled and disabled like normal.
     * For custom patches, you can use the `NOAHH_MOD_STATIC_PATCH` macro to add a patch. This patch will be applied by the launcher and can not be disabled.
 ...
 * Major TulipHook update from 2.5.0 to 3.1.0 (#1395)
   ...
   * This update is a mainly internal one, but it is the most sensitive part of Noahh
...
## v4.5.0
 * Multipart form support for Noahh web utilities (#1345)
 ...
 * Add a `noahh::utils::string::pathToString` method, which safely handles UTF-16 conversions on Windows (95eed22, 138d071)
 * Fix base64 utilities not being exported and thus unusable by mods (dcd42df)
 * Fix `noahh::utils::game::exit` and `restart` not linking on all platforms except Windows (50c09b0)
 ...
 * Add `noahh::dirs::getResourcesDir()` and `{gd_resources_dir}` setting variable (#1285)
 * Add `noahh::globalListen()` and `EventListener` operator= move (#1218)
 * Add base64 utils (#1307)
 * Make `noahh::utils::web::openLinkInBrowser` open the system webview on Android (#1295)
 ...
 * Add optional saving for `noahh::utils::game::exit` and `noahh::utils::game::restart` (061c335)
 ...
 * Fix some `noahh::stl` iterator conversions (e64ab20)
 * Expose `BreakLine` class (#1309)
 * Correctly check image size in MacOS crashlogs (#1274)
 * Fix a MacOS crash related to extra keys added by Noahh (f7d44f3)
 ...
 * Event export macro (#1243)
   * See [the docs](https://docs.noahh-sdk.org/mods/dependencies#event-macro) for more info
 ...
 * Fix some bugs in `noahh::utils::ranges` methods (#1236, #1239)
 ...
 * Log levels for console and file logging (#1208)
   * Now in the Noahh settings you can set the minimum severity for logs
 ...
 * Reorganize buttons in Noahh layer on 4:3 aspect ratio screens (54c93c4)
 ...
 * Rename Loader to Noahh in LoadingLayer text (203c819)
 * Add `NOAHH_BUNDLE_PDB` option in CMake to allow bundling PDB files with mods (#1134)
 ...
 * Fix target Noahh version comparison with tags (ec2e8d8)
 ...
 * Add `NOAHH_DESKTOP(...)` and `NOAHH_MOBILE(...)` macros (d6f0c59)
 ...
 * Add `ModMetadata::checkNoahhVersion` and `ModMetadata::checkTargetVersions` (e881dc5)
 * Add `noahh::createModLogo` for creating a logo from a `.noahh` package (e881dc5)
 ...
 * Fix searching for mods ignoring noahh and gd version (#1153)
 ...
 * Allow building noahh itself in debug mode (5645399)

## v4.0.0-alpha.1
 * Support for the 2.2074 update
 * Developers, see [this page for a migration guide](https://docs.noahh-sdk.org/tutorials/migrate-v4)
 * Major API breaks:
   * Remove everything previously marked deprecated
   * `utils::MiniFunction` removed
   * Rewritten `noahh::Result` class
   * Rewritten matjson library
   * Settings V2 completely removed, use V3 now
   * `JsonChecker` removed
 * Add new system for ordered hook priority, [see docs](https://docs.noahh-sdk.org/tutorials/hookpriority) (673317d, 6db3084)
 * C++20 coroutine support for `noahh::Task`, [see docs](https://docs.noahh-sdk.org/tutorials/tasks#coroutines) (e61b2c0, ab196b9)
 * Add `Task::chain`, [see docs](https://docs.noahh-sdk.org/tutorials/tasks#chaining-tasks) (3248831)
 * Single page local mods list (efb1fbf)
 * Split mod problems into load and outdated (12e8bbb, 09fa872, df2528c)
   * This means mods made for outdated gd or noahh versions no longer count as actual errors, resulting in less clutter in the ui
 * Fix safe mode popup on windows showing up when not supposed to (038788b)
 * WebRequest::ignoreContentLength (#1126)
 * Lots of smaller fixes to the noahh ui (c9afa75, f5f3365, 2d66279, 02845d9, 9b95301, 6d13f78, 123b3ab, 0b2fc66, f96ea5e, cad670f)
 ...
 * Add `noahh::utils::string::caseInsensitiveCompare` (f3d38a7)
 ...
 * Fix crash when noahh menu is disabled (b69f810)
...
## v3.7.0
 * Add persistent directory for mods to save data that won't be deleted when the mod/Noahh is uninstalled (68ab475)
...
## v3.6.0
 * Major rework of the entire settings system with lots of new features; see the [docs page](https://docs.noahh-sdk.org/mods/settings) for more
 ...
 * Add `NOAHH_PLATFORM_SHORT_IDENTIFIER_NOARCH` (1032d9a)
 ...
 * Fix missing `NOAHH_DLL` (e4054d4)
 * Add code of conduct (80693c1, ab8ace0, ca3a2a3)
 * Add ID system to Noahh's web requests (#1040, 1f2aa2c, 1b5ae86)
...
## v3.4.0
 * Add an API for modifying the Noahh UI via events; see [the corresponding docs page](https://docs.noahh-sdk.org/tutorials/modify-noahh) (2a3c35f)
 ...
 * Add `NOAHH_DONT_USE_CCACHE` CMake option (2edfb71)
 ...
 * Bring back uninstall Noahh button on Windows (22b2580)
 * Add `noahh::openChangelogPopup` (e432e72)
 ...
 * Windows installer improvements:
   * Delete old noahh/update folder to prevent downgrading (116af49)
...
## v3.0.0-beta.3
 * Fix a crash that might happen when entering the Noahh mods page (79689cd)
 ...
 * Split `NOAHH_PLATFORM_NAME` for Mac ARM and Mac Intel (48a7981)
 ...
 * Add `noahhImplicitEntry` and `noahhCustomEntry` (6b2ac24, 5969c90)
 ...
 * noahh::ui::Border fixes (#883) - thanks SMJS
 ...
 * New UI for the Index, which also uses a server implementation (#833)
   ...
   * Adds 4 tabs to the Noahh menu: Installed, Recommended, Download, Recent
   * Creates a fancy new background with `SwelvyBG`
   * The entire color scheme has been changed to fit Noahh's color language of pastel purple-pastel gold gradient
 ...
 * Add a new `Task` class for generic asynchronous task implementations
   * Documentation can be found [here](https://docs.noahh-sdk.org/tutorials/tasks/)
 ...
 * Add `noahh::cocos::isSpriteName` and `noahh::cocos::getChildBySpriteName` (#725)
 ...
 * Error if mod target noahh version doesn't match installed SDK version (2c559f8)
 ...
 * Prevent `NoahhUpdater.exe` from hanging (d139049b)
 ...
 * Fix Windows forward compatibility mode (eef949c5, 824efbf3, 456075a2)
   * This was caused by Clang not setting `/DELAYLOAD` properly, but also a mutex introduced in beta.7 causing Noahh not to load at all
 ...
 * Add `noahh::openSupportPopup` for opening the Support page for a mod (438252f)
 ...
 * Fix `noahh::web::fetchJSON` and `ColorPickPopup` not being DLL-exported (2c1eb67, 8250a73)
 ...
 * Install resources on local Noahh build (e9aa889)
...
## v2.0.0-beta.14
 * Use Breakpad for crash reports on Android ([#481](https://github.com/noahh-sdk/noahh/pull/481))
...
## v2.0.0-beta.13
 * Add new `AnchorLayout` feature ([#476](https://github.com/noahh-sdk/noahh/pull/476))
 ...
 * No longer consider outdated mods invalid .noahh files altogether (d9c65b37)
 ...
 * Force noahh internal mod to be first on list (c2d1e587)
 * Fix noahh layer eating inputs if search is selected (18983974)
...
## v2.0.0-beta.2
 * Fix `noahh::utils::cocos::reloadTextures` on windows

## v2.0.0-beta.1
 * 2.2 support!
   * Android arm64-v8 support!
   * Future version compatibility mode, where the Noahh UI is disabled
...
## v1.4.0
 * Add Android support !!!!
   * Implement every Noahh functionality except `utils::file::openFolder`
   * Requires the Noahh launcher in order to be used
   ...
   * All Noahh and GD files are stored in `Android/data/com.noahh.launcher/files`
 ...
 * Add uninstall button to Noahh mod (Only functional in Windows currently) (a738320)
 ...
 * Add `NOAHH_HIDDEN` to inline unique functions (71a79ab)
 * Fix big mod icons (26a6c7e)
 * Fix `CCNode::removeChildByID` export (23cd456)
 * Make `MDTextArea` fit its size (140f38b)
 * Enable ESC/Back to go back in Noahh mod list (2847bee)
 * Add `SimpleTextArea` (7f277a7)
 * Check modified date when unzipping `.noahh` files (5c765c6)
 ...
 * Fix NOAHH_CLI force caching when not found (0a113744)
...
## v1.0.3
 * Fix mod changelogs only showing Noahh's own changelog (2945422)
 * Fix some filesystem functions for Noahh running under wine (7a83354)
 ...
 * Fix Noahh not loading when NoahhBootsrapper.dll exists (c2aacd9)
 ...
 * Fix `noahh::utils::file::pickFile` on MacOS (8842e8f)
 * Fix `noahh::utils::file::writeToJson` (1c699c3)
 ...
 * `NOAHH_DEBUG` on longer needs to be defined for `log::debug` to work
 * Make Noahh changelog visible in-game
...
## v1.0.0-beta.11
 * New `noahh::prelude` namespace to replace the old `USE_NOAHH_NAMESPACE()` macro
 * Add `CCNode::removeChildByID`
 * Add `CCNode::hasAncestor`
 * Add `CCScene::get` and `CCScheduler::get`
 * Add `noahh::cocos::getMousePos`
 * Add `NOAHH_DONT_INSTALL_MODS` option to CMake
 ...
 * Add `WeakRef` as a weak pointer alternative to `Ref` (see [the docs](https://docs.noahh-sdk.org/tutorials/memory#weakref) for a tutorial)
 ...
 * `Mod::getResourcesDir` now returns `noahh/unzipped/{mod.id}/resources/{mod.id}` in conjunction with [CLI v2.1.0](https://github.com/noahh-sdk/cli/releases/tag/v2.1.1)
 ...
 * Fix `setup_noahh_mod` not linking to Noahh if CLI calls are disabled
 ...
 * `noahh/unzipped` is now deleted on startup if it exists
 ...
 * Add support for Noahh CLI v2.0.0 (which has not been released yet)
 ...
 * Display Noahh commit hash in the bottom right of the mod info layer
 * Fix `EditorPauseLayer` info labels on the top left being too big

## v1.0.0-beta.6
 * Reworked layouts from the ground up - see [the docs page](https://docs.noahh-sdk.org/tutorials/layouts) for more information about how the new system works (#137)
 ...
 * Move `NOAHH_DLL` to the structs themselves in `JsonValidation`
 ...
 * Noahh's internal mod representation is now included in the loader's loaded mods list
...
## v1.0.0-beta.3
 - Better support for dependencies with [Noahh CLI v1.4.x](https://github.com/noahh-sdk/cli/releases/latest): mod dependencies are now automatically installed from the mods index by simply declaring them in your `mod.json`. See [the tutorial page in docs](https://docs.noahh-sdk.org/mods/dependencies/) for more
 - The `create_noahh_file` CMake function has been replaced by `setup_noahh_mod`. The old `create_noahh_file` function is still available, but will be deprecated in the future
...
## v1.0.0-beta.1
 * Switched to [a new custom-built JSON library](https://github.com/noahh-sdk/json) to replace `nlohmann::json` for compile-time improvements; if you were using the old JSON library, you can add & link to `nlohmann::json` in your own project, or update to use the new API.
...
## v0.6.1
 - Add `noahh::cocos::switchToScene` utility for easily switching to a layer with the default fade transition
 - Add `Mod::getPackagePath` as a replacement for `Mod::getPath`
 - Add `noahh/config` directory as a standardized place for mods to add their config files
...
## v0.6.0
 ...
 - New header `Noahh/ui/LayerBG.hpp` with `createLayerBG` function for creating the blue gradient background GD layers have
 - All Cocos2d include paths have been changed to be relative
 - `cocos2dx` folder has been removed, cocos2d is now directly inside `Noahh/cocos`
 - Loader resources updating will no longer check for plist file differences due to problems with CLI on Mac
 - More codegen optimizations, Noahh mods should now compile faster
...
## v0.5.0
 ...
 - Deprecate `noahh::utils::vector` and `noahh::utils::container` namespaces and replace them with `noahh::utils::ranges`
 - Finally added a license to Noahh! The framework is now licensed under BSL v1.0.
...
## v0.4.6
 ...
 - CMake rework; `NoahhFile.cmake` now checks and verifies CLI version
 - Add optional `DONT_INSTALL` parameter to `create_noahh_file`
 - Test mods are now no longer automatically installed
 - Add `package_noahh_resources_now` command for packaging resources at configure time and creating a header with their calculated hashes
 - Fix `getSceneDelegate`
 - Change `CCArrayExt` to use `Ref`

## v0.4.5
 - Rework bindings and codegen to improve compile times, now individual bindings can be included with `<Noahh/binding/{ClassName}.hpp>`
 - Modify has also been separated, you can now include individual modifiers with `<Noahh/modify/{ClassName}.hpp>`
 ...
 - Add `NOAHH_DEBUG` macro for enabling `log::debug` to actually print stuff
 - Show crashlog on crash when `NOAHH_DEBUG` is enabled
 - Add `JsonChecker::at` and `JsonChecker::array` for dealing with arrays
 - Add `noahh::utils::web::fetchBytes` for fetching a byte array synchronously
 - Add `noahh::utils::web::AsyncWebRequest` for creating thread-safe asynchronous web requests
 - Add `Loader::updateModResourcePaths` for adding a mods' resources to search paths. Not recommended to be called manually
 - Add an overload to `noahh::createQuickPopup` for specifying popup width
...
## v0.4.3
 ...
 - Add `Noahh/ui/NoahhUI.hpp` header for exposing some access to internal Noahh UI like opening a mod's settings popup
...
## 2022/07/30
 - `sdk`, `loader` and `api` repos all merged into one `noahh` repo

## 2022/05/24
 - Noahh announced to be merging with Hyperdash, later on it turned out we were all pathological liars
...
## 2022/01/19
 - Lilac and Cacao merged and renamed to Noahh
