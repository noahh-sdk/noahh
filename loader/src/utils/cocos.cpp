#include <Noahh/modify/LoadingLayer.hpp>
#include <Noahh/utils/cocos.hpp>
#include <matjson.hpp>
#include <Noahh/binding/CCTextInputNode.hpp>
...
Result<cocos2d::ccColor3B, std::string> matjson::Serialize<ccColor3B>::fromJson(matjson::Value const& value) {
    if (value.isArray()) {
        auto arr = NOAHH_UNWRAP(value.asArray());
        if (arr.size() == 3) {
            auto r = NOAHH_UNWRAP(arr[0].asInt());
            auto g = NOAHH_UNWRAP(arr[1].asInt());
            auto b = NOAHH_UNWRAP(arr[2].asInt());
            return Ok(cocos2d::ccc3(r, g, b));
        }
        return Err("Expected color array to have 3 items");
    }
    if (value.isObject()) {
        auto r = NOAHH_UNWRAP(NOAHH_UNWRAP(value.get("r")).asInt());
        auto g = NOAHH_UNWRAP(NOAHH_UNWRAP(value.get("g")).asInt());
        auto b = NOAHH_UNWRAP(NOAHH_UNWRAP(value.get("b")).asInt());
        return Ok(cocos2d::ccc3(r, g, b));
    }
    if (value.isString()) {
        auto hex = NOAHH_UNWRAP(value.asString());
...
Result<cocos2d::ccColor4B, std::string> matjson::Serialize<ccColor4B>::fromJson(matjson::Value const& value) {
    if (value.isArray()) {
        ...
        if (arr.size() == 4) {
            ...
            auto g = NOAHH_UNWRAP(arr[1].asInt());
            auto b = NOAHH_UNWRAP(arr[2].asInt());
            auto a = NOAHH_UNWRAP(arr[3].asInt());
    ...
    if (value.isObject()) {
        ...
        auto g = NOAHH_UNWRAP(NOAHH_UNWRAP(value.get("g")).asInt());
        auto b = NOAHH_UNWRAP(NOAHH_UNWRAP(value.get("b")).asInt());
        auto a = NOAHH_UNWRAP(NOAHH_UNWRAP(value.get("a")).asInt());
        return Ok(cocos2d::ccc4(r, g, b, a));
    }
    if (value.isString()) {
        auto hex = NOAHH_UNWRAP(value.asString());
...
Result<ccColor3B> noahh::cocos::cc3bFromHexString(std::string_view hexValue, bool permissive) {
...
Result<ccColor4B> noahh::cocos::cc4bFromHexString(std::string_view hexValue, bool requireAlpha, bool permissive) {
    if (permissive && hexValue.empty()) {
...
std::string noahh::cocos::cc3bToHexString(ccColor3B const& color) {
...
std::string noahh::cocos::cc4bToHexString(ccColor4B const& color) {
    static constexpr auto digits = "0123456789ABCDEF";
    std::string output;
    output += digits[color.r >> 4 & 0xF];
    output += digits[color.r & 0xF];
...
bool noahh::cocos::isSpriteFrameName(CCNode* node, const char* name) {
...
CCNode* noahh::cocos::getChildBySpriteFrameName(CCNode* parent, const char* name) {
...
bool noahh::cocos::isSpriteName(CCNode* node, const char* name) {
    if (!node) return false;
...
CCNode* noahh::cocos::getChildBySpriteName(CCNode* parent, const char* name) {
    for (auto child : CCArrayExt<CCNode*>(parent->getChildren())) {
...
std::string_view noahh::cocos::getObjectName(cocos2d::CCObject const* obj) {
#ifdef NOAHH_IS_WINDOWS
...
CCRect noahh::cocos::calculateNodeCoverage(std::span<CCNode*> nodes) {
    CCRect coverage;
...
CCRect noahh::cocos::calculateNodeCoverage(CCArray* nodes) {
    CCRect coverage;
...
CCRect noahh::cocos::calculateChildCoverage(CCNode* parent) {
    return calculateNodeCoverage(parent->getChildren());
}

void noahh::cocos::limitNodeSize(CCNode* spr, CCSize const& size, float def, float min) {
    spr->setScale(std::clamp(std::min(size.height / spr->getContentHeight(), size.width / spr->getContentWidth()), min, def));
}

void noahh::cocos::limitNodeWidth(CCNode* spr, float width, float def, float min) {
    spr->setScale(std::clamp(width / spr->getContentSize().width, min, def));
}

void noahh::cocos::limitNodeHeight(CCNode* spr, float height, float def, float min) {
    spr->setScale(std::clamp(height / spr->getContentSize().height, min, def));
}

CCSize noahh::cocos::getLabelSize(std::u16string_view text, const char* font, int kerning) {
...
CCSize noahh::cocos::getLabelSize(std::string_view text, const char* font, int kerning) {
...
bool noahh::cocos::nodeIsVisible(CCNode* node) {
    if (!node->isVisible()) {
        return false;
    }
...
CCNode* noahh::cocos::getChildByTagRecursive(cocos2d::CCNode* node, int tag) {
...
bool noahh::cocos::fileExistsInSearchPaths(char const* filename) {
    auto utils = CCFileUtils::sharedFileUtils();
    return utils->isFileExist(utils->fullPathForFilename(filename, false));
}

CCScene* noahh::cocos::switchToScene(CCLayer* layer) {
...
void noahh::cocos::reloadTextures(CreateLayerFunc returnTo) {
    LOADING_FINISHED_SCENE = std::move(returnTo);
    GameManager::get()->reloadAll(false, false, true);
}

void NOAHH_DLL noahh::cocos::handleTouchPriorityWith(cocos2d::CCNode* node, int priority, bool force) {
...
void NOAHH_DLL noahh::cocos::handleTouchPriority(cocos2d::CCNode* node, bool force) {
...
struct LoadingFinished : Modify<LoadingFinished, LoadingLayer> {
    NOAHH_FORWARD_COMPAT_DISABLE_HOOKS("noahh::cocos::reloadTextures disabled")
