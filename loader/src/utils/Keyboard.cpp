...
std::string Keybind::toString() const {
    ...
    if ((modifiers & KeyboardModifier::Super) && key != KEY_LeftWindowsKey && key != KEY_RightWindowsKey) {
        #if defined(NOAHH_IS_MACOS) || defined(NOAHH_IS_IOS)
        buf.append("Cmd+");
...
Result<Keybind> matjson::Serialize<noahh::Keybind>::fromJson(matjson::Value const& json) {
    if (!json.isObject()) {
...
matjson::Value matjson::Serialize<noahh::Keybind>::toJson(Keybind const& value) {
