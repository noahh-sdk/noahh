...
Result<VersionInfo> VersionInfo::parse(std::string string) {
    ...
    if (str.peek() == '-') {
        str.get();
        NOAHH_UNWRAP_INTO(tag, VersionTag::parse(str));
    }
...
std::string noahh::format_as(VersionInfo const& version) {
...
Result<ComparableVersionInfo> ComparableVersionInfo::parse(std::string string) {
    ...
    NOAHH_UNWRAP_INTO(auto version, VersionInfo::parse(std::move(string)));
...
std::string noahh::format_as(ComparableVersionInfo const& version) {
    return version.toString();
}

// returns true if current version can run target version
bool noahh::semverCompare(VersionInfo const& current, VersionInfo const& target) {
    // Noahh v1.0.0, target v2.0.0 -> false
    if (target.getMajor() != current.getMajor()) {
        return false;
    }
    // Noahh v1.0.0, target v1.1.0 -> false
    ...
    if (ct && tt) {
        ...
        switch (targetTag.value) {
            case VersionTag::Alpha:
                // Noahh v1.0.0-beta, target v1.0.0-alpha -> false
                if (currentTag.value > VersionTag::Alpha) {
                    return false;
                }
                // Noahh v1.0.0-alpha.1, target v1.0.0-alpha.2 -> false
                if (currentTag.number && targetTag.number) {
                    return currentTag.number.value() == targetTag.number.value();
                }
                // Noahh v1.0.0-alpha.1, target v1.0.0-alpha -> true
                if (currentTag.number) {
                    return true;
                }
                // Noahh v1.0.0-alpha, target v1.0.0-alpha.1 -> false
                if (targetTag.number) {
                    return false;
                }
                return true;
            case VersionTag::Beta:
                // Noahh v1.0.0-alpha, target v1.0.0-beta -> false
                if (currentTag.value <= VersionTag::Alpha) {
                    return false;
                }
                // Noahh v1.0.0-beta.2, target v1.0.0-beta.1 -> true
                // Noahh v1.0.0-beta.4, target v1.0.0-beta.5 -> false
                if (currentTag.number && targetTag.number) {
                    return currentTag.number.value() >= targetTag.number.value();
                }
                // Noahh v1.0.0-beta.1, target v1.0.0-beta -> true
                if (currentTag.number) {
                    return true;
                }
                // Noahh v1.0.0-beta, target v1.0.0-beta.1 -> false
                if (targetTag.number) {
                    return false;
                }
                return true;
            default:
                // Noahh v1.0.0-alpha, target v1.0.0-prerelease -> false
                if (currentTag.value <= VersionTag::Alpha) {
                    return false;
                }
                // Noahh v1.0.0-prerelease.2, target v1.0.0-prerelease.3 -> true
                return true;
        }
    }
    else if (ct) {
        auto currentTag = ct.value();
        // Noahh v1.0.0-beta, target v1.0.0 -> true
        // if (currentTag.value > VersionTag::Alpha) {
        //     return true;
        // }
        return false;
        ...
        auto targetTag = tt.value();
        // Noahh v1.0.0, target v1.0.0-beta -> true
