...
using namespace noahh::utils::web;
using namespace noahh::utils::string;
using namespace arc;
...
public:
    ...
    void pickUniqueBoundary() const {
        ...
        do {
            m_boundary = fmt::format("----NoahhWebBoundary{}", timestamp++);
...
Result<MultipartForm&> MultipartForm::file(std::string name, std::filesystem::path const& path, std::string mime) {
    if (!m_impl->isBuilt()) {
        ...
        NOAHH_UNWRAP_INTO(auto data, utils::file::readBinary(path));
...
bool WebResponse::cancelled() const {
    return m_impl->m_code == static_cast<int>(NoahhWebError::REQUEST_CANCELLED);
}
...
Result<matjson::Value> WebResponse::json() const {
    NOAHH_UNWRAP_INTO(auto value, this->string());
...
public:
    ...
    struct RequestData : std::enable_shared_from_this<RequestData> {
        ...
        noahh::Function<void(WebResponse)> onComplete;
        CURL* curl = nullptr;

        RequestData(std::shared_ptr<WebRequest::Impl> req, Mod* mod, size_t id, noahh::Function<void(WebResponse)> cb)
        ...
        void onError(NoahhWebError code, std::string_view msg) {
...
public:
    ...
    std::vector<noahh::Function<void(WebProgress const&)>> m_progressCallbacks;
    ...
    WebResponse makeError(NoahhWebError code, std::string_view msg) {
        auto res = WebResponse();
...
static arc::Future<> dohProbe() {
    auto resp = co_await web::WebRequest{}
        ...
        .get("https://cloudflare-dns.com/dns-query?name=api.noahh-sdk.org");
...
public:
    ...
    Impl() {
        ...
        m_worker.setName("Noahh Web Worker");
    }
    ...
    void workerAddRequest(std::shared_ptr<RequestData> req) {
        ...
        if (!handle) {
            req->onError(NoahhWebError::CURL_INITIALIZATION_ERROR, "Failed to initialize cURL");
            return;
        }
    ...
    void workerCancelRequest(std::shared_ptr<RequestData> req) {
        ...
        req->onError(NoahhWebError::REQUEST_CANCELLED, "Request cancelled");
...
WebFuture::~WebFuture() {
    ...
    if (!m_impl->m_sent) {
        m_impl->m_request->onError(NoahhWebError::REQUEST_CANCELLED, "Request cancelled");
...
std::optional<WebResponse> WebFuture::poll(arc::Context& cx) {
    if (!m_impl->m_sent) {
        ...
        if (!res) {
            return m_impl->m_request->request->makeError(NoahhWebError::QUEUE_FULL, "Failed to enqueue web request: queue is full");
        }
        m_impl->m_sent = true;
    }
    ...
    if (!result) {
        ...
        return m_impl->m_request->request->makeError(NoahhWebError::CHANNEL_CLOSED, "Failed to receive web response: channel closed");
    }
