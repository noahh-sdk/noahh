...
#include <Noahh/binding/FLAlertLayerProtocol.hpp>
...
// void noahh::enableDynamicLayoutForPopup(FLAlertLayer* alert, CCNode* bg) {
//     auto winSize = CCDirector::get()->getWinSize();
...
bool Popup::init(
    ...
    m_buttonMenu->addChildAtPosition(m_closeBtn, noahh::Anchor::TopLeft, { 3.f, -3.f });
...
protected:
    noahh::Function<void(FLAlertLayer*, bool)> m_selected;
...
public:
    static QuickPopup* create(
        ...
        float width, noahh::Function<void(FLAlertLayer*, bool)> selected, bool cancelledByEscape
    ) {
...
FLAlertLayer* noahh::createQuickPopup(
    char const* title, std::string content, char const* btn1, char const* btn2, float width,
    noahh::Function<void(FLAlertLayer*, bool)> selected, bool doShow, bool cancelledByEscape
) {
