...
#include <Noahh/binding/TextInputDelegate.hpp>
#include <Noahh/modify/CCTextInputNode.hpp>
...
struct TextInputNodeFix : Modify<TextInputNodeFix, CCTextInputNode> {
    NOAHH_FORWARD_COMPAT_DISABLE_HOOKS("TextInputNode fix")
...
const char* noahh::getCommonFilterAllowedChars(CommonFilter filter) {
    switch (filter) {
        default:
...
public:
    ...
    noahh::Function<void(std::string const&)> onInput = nullptr;
...
void TextInput::setCallback(noahh::Function<void(std::string const&)> onInput) {
