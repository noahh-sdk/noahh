...
void noahh::openModsList() {
    ModsLayer::scene();
}

void noahh::openIssueReportPopup(Mod* mod) {
    if (mod->getMetadata().getIssues()) {
        ...
        MDPopup::create(
            ...
            fmt::format(
                ...
                "channel in the [Noahh Discord Server](https://discord.gg/9e43WMKzhp)\n\n"
                "If your issue relates to a <cr>game crash</c>, <cb>please include</c> the "
                "latest crash log(s) from `{}`",
                dirs::getCrashlogsDir()
            ),
...
void noahh::openSupportPopup(Mod* mod) {
    openSupportPopup(mod->getMetadata());
}

void noahh::openSupportPopup(ModMetadata const& metadata) {
    MDPopup::create(
...
void noahh::openInfoPopup(Mod* mod) {
    ModPopup::create(mod)->show();
}
std::optional<arc::TaskHandle<bool>> noahh::openInfoPopup(std::string modID) {
...
void noahh::openChangelogPopup(Mod* mod) {
    auto popup = ModPopup::create(mod);
    popup->loadTab(ModPopup::Tab::Changelog);
    popup->show();
}

void noahh::openSettingsPopup(Mod* mod) {
    openSettingsPopup(mod, true);
}
Popup* noahh::openSettingsPopup(Mod* mod, bool disableNoahhTheme) {
    if (mod->hasSettings()) {
        auto popup = ModSettingsPopup::create(mod, disableNoahhTheme);
        popup->show();
        return popup;
    }
    return nullptr;
            ...
            [this](Mod* mod) {
                ...
                if (!mod->isInternal()) {
                    ...
                    if (Mod::get()->getSavedValue("alternate-noahh-style", false)) {
                        m_sprite->initWithSpriteFrameName("noahh-logo-alternate.png"_spr);
                    }
                    else {
                        m_sprite->initWithSpriteFrameName("noahh-logo.png"_spr);
                    }
                }
            },
...
CCNode* noahh::createDefaultLogo() {
    return ModLogoSprite::create(ModLogoSrc(nullptr));
}

CCNode* noahh::createModLogo(Mod* mod) {
    return ModLogoSprite::create(ModLogoSrc(mod));
}

CCNode* noahh::createModLogo(std::filesystem::path const& noahhPackage) {
    return ModLogoSprite::create(ModLogoSrc(noahhPackage));
}

CCNode* noahh::createServerModLogo(std::string id) {
