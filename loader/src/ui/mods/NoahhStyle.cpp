...
#include <Noahh/ui/LoadingSpinner.hpp>
...
$on_mod(Loaded) {
    ...
    if (Mod::get()->getSavedValue("alternate-noahh-style", false)) {
    ...
    listenForSettingChanges<bool>("enable-noahh-theme", updateColors);
    ...
    Loader::get()->queueInMainThread([updateColors = updateColors] {
        ...
        updateColors(Mod::get()->getSettingValue<bool>("enable-noahh-theme"));
    });
}

bool isNoahhTheme(bool forceDisableTheme) {
    return !forceDisableTheme && Mod::get()->getSettingValue<bool>("enable-noahh-theme");
}

bool NoahhPopup::init(float width, float height, NoahhPopupStyle style, bool forceDisableTheme) {
    m_forceDisableTheme = forceDisableTheme;

    const bool noahhTheme = isNoahhTheme(forceDisableTheme);
    const char* bg;
    switch (style) {
        default:
        case NoahhPopupStyle::Default: bg = noahhTheme ? "GE_square01.png"_spr : "GJ_square01.png"; break;
        case NoahhPopupStyle::Alt:     bg = noahhTheme ? "GE_square02.png"_spr : "GJ_square02.png"; break;
        case NoahhPopupStyle::Alt2:    bg = noahhTheme ? "GE_square03.png"_spr : "GJ_square02.png"; break;
    }
    ...
    this->setCloseButtonSpr(
        CircleButtonSprite::createWithSpriteFrameName(
            "close.png"_spr, .85f,
            (noahhTheme ?
                (style == NoahhPopupStyle::Default ? CircleBaseColor::DarkPurple : CircleBaseColor::DarkAqua) :
...
bool NoahhSquareSprite::init(CCSprite* top, bool* state, bool forceDisableTheme) {
    if (!CCSprite::initWithFile(isNoahhTheme(forceDisableTheme) ? "GE_button_05.png"_spr : "GJ_button_01.png"))
        return false;
...
void NoahhSquareSprite::updateImage() {
    this->setTexture(CCTextureCache::get()->addImage(
        (m_state ? "GJ_button_02.png" : (isNoahhTheme(m_forceDisableTheme) ? "GE_button_05.png"_spr : "GJ_button_01.png")),
        false
    ));
}
void NoahhSquareSprite::update(float dt) {
    CCSprite::update(dt);
...
NoahhSquareSprite* NoahhSquareSprite::create(const char* top, bool* state, bool forceDisableTheme) {
    auto ret = new NoahhSquareSprite();
...
NoahhSquareSprite* NoahhSquareSprite::createWithSpriteFrameName(const char* top, bool* state, bool forceDisableTheme) {
    auto ret = new NoahhSquareSprite();
...
CCSprite* NoahhSquareSprite::getTopSprite() const {
    return m_topSprite;
}

void NoahhSquareSprite::setState(bool state) {
    if (!m_stateSrc) {
        m_state = state;
        this->updateImage();
    }
...
const char* getNoahhButtonSpriteName(NoahhButtonSprite spr, bool forceDisableTheme) {
    if (isNoahhTheme(forceDisableTheme)) {
        switch (spr) {
            default:
            case NoahhButtonSprite::Default: return "GE_button_05.png"_spr;
            case NoahhButtonSprite::Install: return "GE_button_01.png"_spr;
            case NoahhButtonSprite::Delete: return "GJ_button_06.png";
            case NoahhButtonSprite::Enable: return "GJ_button_01.png";
            case NoahhButtonSprite::Gray: return "GJ_button_05.png";
        }
    }
    else {
        switch (spr) {
            default:
            case NoahhButtonSprite::Default: return "GJ_button_01.png";
            case NoahhButtonSprite::Install: return "GE_button_01.png"_spr;
            case NoahhButtonSprite::Delete: return "GJ_button_06.png";
            case NoahhButtonSprite::Enable: return "GJ_button_02.png";
            case NoahhButtonSprite::Gray: return "GJ_button_05.png";
        }
    }
}

IconButtonSprite* createNoahhButton(CCNode* icon, ZStringView text, bool gold, NoahhButtonSprite bg, bool forceDisableTheme) {
    return IconButtonSprite::create(
        getNoahhButtonSpriteName(bg, forceDisableTheme),
        icon, text.c_str(),
        gold ? "goldFont.fnt" : "bigFont.fnt"
    );
}
IconButtonSprite* createNoahhButton(CCNode* icon, ZStringView text, NoahhButtonSprite bg, bool forceDisableTheme) {
    return createNoahhButton(icon, text, false, bg, forceDisableTheme);
}
ButtonSprite* createNoahhButton(ZStringView text, int width, bool gold, bool absolute, NoahhButtonSprite bg, bool forceDisableTheme) {
    return ButtonSprite::create(text.c_str(), width, absolute, gold ? "goldFont.fnt" : "bigFont.fnt", getNoahhButtonSpriteName(bg, forceDisableTheme), 0.0f, .8f);
}
ButtonSprite* createNoahhButton(ZStringView text, bool gold, NoahhButtonSprite bg, bool forceDisableTheme) {
    return ButtonSprite::create(text.c_str(), gold ? "goldFont.fnt" : "bigFont.fnt", getNoahhButtonSpriteName(bg, forceDisableTheme), .8f);
}

CircleButtonSprite* createNoahhCircleButton(CCSprite* top, float scale, CircleBaseSize size, bool altColor, bool forceDisableTheme) {
    const auto noahhTheme = isNoahhTheme(forceDisableTheme);
    auto ret = CircleButtonSprite::create(
        top, noahhTheme ? (altColor ? CircleBaseColor::DarkAqua : CircleBaseColor::DarkPurple) : CircleBaseColor::Green, size
    );
...
ButtonSprite* createNoahhTagLabel(server::ServerTag const& tag) {
    return createTagLabel(tag.displayName, noahhTagColors(tag));
}
...
ListBorders* createNoahhListBorders(CCSize const& size, bool forceDisableTheme) {
    auto ret = ListBorders::create();
    const bool noahhTheme = isNoahhTheme(forceDisableTheme);
    if (noahhTheme) {
        ret->setSpriteFrames("noahh-list-top.png"_spr, "noahh-list-side.png"_spr, 2);
...
bool NoahhTabSprite::init(const char* iconFrame, const char* text, float width, bool altColor) {
    if (!CCNode::init())
        return false;
...
NoahhTabSprite* NoahhTabSprite::create(const char* iconFrame, const char* text, float width, bool altColor) {
    auto ret = new NoahhTabSprite();
...
void NoahhTabSprite::select(bool selected) {
    m_deselectedBG->setVisible(!selected);
    m_selectedBG->setVisible(selected);
}

void NoahhTabSprite::disable(bool disabled) {
