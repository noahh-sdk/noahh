#include "ModList.hpp"
#include <Noahh/cocos/actions/CCActionInterval.h>
...
bool ModList::init(ModListSource* src, CCSize const& size, bool searchingDev) {
    ...
    // Noahh can't tell if it's looking for a dev due to a source cache or due to actually coming from pressing more when creating a new list
    ...
    if (typeinfo_cast<InstalledModListSource*>(m_source)) {
        ...
        m_showUpdatesSpr = createNoahhButton(
            CCSprite::createWithSpriteFrameName("GJ_filterIcon_001.png"),
            "Show Updates", NoahhButtonSprite::Install
        );
        m_hideUpdatesSpr = createNoahhButton(
            CCSprite::createWithSpriteFrameName("GJ_filterIcon_001.png"),
            "Hide Updates", NoahhButtonSprite::Default
        );
        ...
        m_updateAllSpr = createNoahhButton(
            CCSprite::createWithSpriteFrameName("update.png"_spr),
            "Update All", NoahhButtonSprite::Install
        );
        ...
        if (Loader::get()->getLoadProblems().size()) {
            ...
            auto showErrorsSpr = createNoahhButton(
                CCSprite::createWithSpriteFrameName("GJ_filterIcon_001.png"),
                "Show Errors Only", NoahhButtonSprite::Delete
            );
            auto hideErrorsSpr = createNoahhButton(
                CCSprite::createWithSpriteFrameName("GJ_filterIcon_001.png"),
                "Hide Errors Only", NoahhButtonSprite::Default
            );
    ...
    auto sortSpr = NoahhSquareSprite::createWithSpriteFrameName("GJ_sortIcon_001.png");
    ...
    m_filtersBtn = CCMenuItemSpriteExtra::create(
        NoahhSquareSprite::createWithSpriteFrameName("GJ_filterIcon_001.png"),
    ...
    if (!serverSource || serverSource->getType() != ServerModListType::Modtober) {
        ...
        m_clearFiltersBtn = CCMenuItemSpriteExtra::create(
            NoahhSquareSprite::createWithSpriteFrameName("GJ_deleteIcon_001.png"),
        ...
        auto aboutSpr = createNoahhButton("About");
...
void ModList::updateState() {
    ...
    if (!serverSource || serverSource->getType() != ServerModListType::Modtober) {
        auto filterSpr = static_cast<NoahhSquareSprite*>(m_filtersBtn->getNormalImage());
        filterSpr->setState(!isDefaultQuery);

        auto clearSpr = static_cast<NoahhSquareSprite*>(m_clearFiltersBtn->getNormalImage());
