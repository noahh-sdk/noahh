...
#include <ui/mods/NoahhStyle.hpp>
...
bool ModItem::init(ModSource&& source) {
    ...
    if (auto serverMod = m_source.asServer(); serverMod != nullptr) {
        ...
        auto noahhValid = Loader::get()->isModVersionSupported(version.getNoahhVersion());
        auto gameVersion = version.getGameVersion();
        auto gdValid = !gameVersion || gameVersion == "*" || gameVersion == NOAHH_STR(NOAHH_GD_VERSION);

        if (!noahhValid || !gdValid) {
            spr = createNoahhButton("N/A", 50, false, true, NoahhButtonSprite::Gray);
        }
    }

    if (!spr) {
        if (Loader::get()->isModInstalled(m_source.getID())) {
            spr = createNoahhButton("View", 50, false, true);
        } else {
            spr = createNoahhButton("Get", 50, false, true, NoahhButtonSprite::Install);
        }
    }
    ...
    m_source.visit(makeVisitor {
        [this](Mod* mod) {
            ...
            if (mod->getLoadProblem()) {
                auto viewErrorSpr = createNoahhCircleButton(
    ...
    auto updateSpr = createNoahhCircleButton(
...
void ModItem::updateState() {
    ...
    m_source.visit(makeVisitor {
        ...
        [this](server::ServerModMetadata const& metadata) {
            m_bg->setColor(isNoahhTheme() ? ccWHITE : ccBLACK);
            m_bg->setOpacity(isNoahhTheme() ? 25 : 90);
            ...
            if (isNoahhTheme() && metadata.featured) {
        ...
        if (!wantsRestart && targetsOutdated && !isDownloading && !update.deprecation) {
            ...
            if (m_display == ModListDisplay::Grid) {
                ...
                if (!m_source.getMetadata().checkGameVersion()) {
                    ...
                    m_outdatedLabel->setString(fmt::format(
                        "Outdated (Noahh {})", m_source.getMetadata().getNoahhVersion().toNonVString()
                    ).c_str());
                }
            }
        }
...
void ModItem::onView(CCObject*) {
    ...
    if (m_source.asServer() && m_source.asServer()->tags.contains("paid") && !shownPaidNotif) {
        ...
        return FLAlertLayer::create(
            ...
            "<cy>Noahh does not handle any payments. The mod handles all transactions in their own way.</c>\n\n"
    ...
    if (m_source.asServer()) {
        ...
        if (gameVersion && gameVersion != "*" && gameVersion != NOAHH_STR(NOAHH_GD_VERSION)) {
            return FLAlertLayer::create(
                nullptr,
                "Unavailable",
        ...
        if (!Loader::get()->isModVersionSupported(version.getNoahhVersion())) {
            return FLAlertLayer::create(
                nullptr,
                "Unavailable",
                "This mod targets an <cr>unsupported version of Noahh</c>.",
                "OK", nullptr, 360
            )->show();
        }
    }
    ...
    if (auto mod = m_source.asMod()) {
        ...
        if (auto dep = m_source.hasUpdates().deprecation) {
            ...
            switch (problem->type) {
                ...
                case LoadProblem::Type::InvalidNoahhFile: title = "Invalid Noahh File"; break;
