...
bool FiltersPopup::init(ModListSource* src) {
    ...
    if (!NoahhPopup::init(350.f, height))
        return false;
    ...
    auto okSpr = createNoahhButton("OK", false, NoahhButtonSprite::Default, m_forceDisableTheme);
    okSpr->setScale(.7f);
...
void FiltersPopup::onLoadTags(server::ServerResult<std::vector<server::ServerTag>> result) {
    if (result.isOk()) {
        ...
        for (auto& tag : tags) {
            auto offSpr = createNoahhTagLabel(tag);
            offSpr->m_BGSprite->setOpacity(105);
            offSpr->m_label->setOpacity(105);
            auto onSpr = createNoahhTagLabel(tag);
