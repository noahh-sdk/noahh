...
#include <Noahh/loader/ModSettingsManager.hpp>
...
bool ModPopup::init(ModSource&& src) {
    auto style = src.asServer() ? NoahhPopupStyle::Alt : NoahhPopupStyle::Default;

    if (!NoahhPopup::init(440.f, 280.f, style))
        return false;
    ...
    auto isNoahh = src.asMod() == Mod::get();
    if (isNoahh) {
    ...
    if (isNoahh) {
        auto counter = Mod::get()->getSavedValue("alternate-noahh-style", false) ? 3 : 0;
        auto* logoButton = CCMenuItemExt::createSpriteExtra(getLogo(), [this, counter, getLogo](auto* item) mutable {
            counter++;
            if (counter % 6 == 3) {
                Mod::get()->setSavedValue("alternate-noahh-style", true);
                ...
                Mod::get()->getSaveContainer().erase("alternate-noahh-style");
                for (int i = 0; i < 6; i++) {
    ...
    auto updateModSpr = createNoahhButton(
        CCSprite::createWithSpriteFrameName("update.png"_spr),
        "Update",
        NoahhButtonSprite::Install,
        m_forceDisableTheme
    );
    ...
    auto enableModOffSpr = createNoahhButton(
        CCSprite::createWithSpriteFrameName("GJ_completesIcon_001.png"),
        "Enable",
        NoahhButtonSprite::Enable,
        m_forceDisableTheme
    );
    enableModOffSpr->setScale(.5f);
    auto enableModOnSpr = createNoahhButton(
        CCSprite::createWithSpriteFrameName("GJ_deleteIcon_001.png"),
        "Disable",
        NoahhButtonSprite::Delete,
        m_forceDisableTheme
    );
    ...
    auto reenableModOffSpr = createNoahhButton(
        CCSprite::createWithSpriteFrameName("reset.png"_spr),
        "Re-Enable",
        NoahhButtonSprite::Default,
        m_forceDisableTheme
    );
    reenableModOffSpr->setScale(.5f);
    auto reenableModOnSpr = createNoahhButton(
        CCSprite::createWithSpriteFrameName("reset.png"_spr),
        "Re-Disable",
        NoahhButtonSprite::Default
    );
    ...
    auto unavailableSpr = createNoahhButton(
        CCSprite::createWithSpriteFrameName("exclamation.png"_spr),
        "Unavailable",
        NoahhButtonSprite::Gray,
        m_forceDisableTheme
    );
    ...
    auto installModSpr = createNoahhButton(
    ...
    auto uninstallModSpr = createNoahhButton(
    ...
    auto cancelDownloadSpr = createNoahhButton(
        CCSprite::createWithSpriteFrameName("GJ_deleteIcon_001.png"),
        "Cancel",
        NoahhButtonSprite::Default,
        m_forceDisableTheme
    ...
    for (auto mdTab : std::initializer_list<std::tuple<const char*, const char*, const char*, Tab>> {
        ...
        auto spr = NoahhTabSprite::create(std::get<0>(mdTab), std::get<1>(mdTab), 140, m_source.asServer());
    ...
    auto modUrl = fmt::format("https://noahh-sdk.org/mods/{}", m_source.getID());
    ...
    auto settingsSpr = createNoahhCircleButton(CCSprite::createWithSpriteFrameName("settings.png"_spr), 1.f, CircleBaseSize::Medium, false, m_forceDisableTheme);
...
void ModPopup::onLoadTags(server::ServerResult<std::vector<server::ServerTag>> result) {
    if (result.isOk()) {
        ...
        for (auto& tag : data) {
            m_tags->addChild(createNoahhTagLabel(tag));
        }
        ...
        // this contest blah blah this mod was made for it"
            ...
            auto aboutSpr = createNoahhButton("About", false, NoahhButtonSprite::Default, m_forceDisableTheme);
...
bool ModPopup::availableForInstall() const {
    ...
    if (auto serverSource = m_source.asServer()) {
        ...
        if (
            (gameVersion == "0.000") ||
            (gameVersion && gameVersion != "*" && gameVersion != NOAHH_STR(NOAHH_GD_VERSION)) ||
            (!Loader::get()->isModVersionSupported(version.getNoahhVersion()))) {
            return false;
        }
        return true;
    }
