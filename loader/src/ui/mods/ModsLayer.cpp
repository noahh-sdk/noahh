...
#include <Noahh/cocos/cocoa/CCObject.h>
...
#include <Noahh/binding/Slider.hpp>
#include <Noahh/binding/SetTextPopup.hpp>
#include <Noahh/binding/SetIDPopup.hpp>
...
bool ModsStatusNode::init() {
    ...
    auto restartSpr = createNoahhButton("Restart Now");
    ...
    Mod::get()->setSavedValue<bool>("has-used-noahh-before", true);
...
bool ModsLayer::init() {
    ...
    auto safeArea = noahh::utils::getSafeAreaRect();

    auto winSize = CCDirector::get()->getWinSize();
    const bool isSafeMode = LoaderImpl::get()->isSafeMode();

    const bool noahhTheme = isNoahhTheme();
    if (!isSafeMode) {
        if (noahhTheme) {
    ...
    auto reloadSpr = createNoahhCircleButton(
    ...
    auto settingsSpr = createNoahhCircleButton(
    ...
    auto keybindsSpr = createNoahhCircleButton(
    ...
    if (Mod::get()->getSettingValue<bool>("restart-button")) {
        ...
        auto restartGDCircleSpr = createNoahhCircleButton(
            restartGDSpr, 1.f,
            CircleBaseSize::Medium
        );
    ...
    auto tabsTop = CCSprite::createWithSpriteFrameName(noahhTheme ? "mods-list-top.png"_spr : "mods-list-top-gd.png"_spr);
    tabsTop->setID("frame-top-sprite");
    tabsTop->setAnchorPoint({ .5f, .0f });
    tabsTop->setZOrder(1);
    m_frame->addChildAtPosition(tabsTop, Anchor::Top, ccp(0, -2));

    auto tabsLeft = CCSprite::createWithSpriteFrameName(noahhTheme ? "mods-list-side.png"_spr : "mods-list-side-gd.png"_spr);
    ...
    auto tabsRight = CCSprite::createWithSpriteFrameName(noahhTheme ? "mods-list-side.png"_spr : "mods-list-side-gd.png"_spr);
    ...
    auto tabsBottom = CCSprite::createWithSpriteFrameName(noahhTheme ? "mods-list-bottom.png"_spr : "mods-list-bottom-gd.png"_spr);
    ...
    for (auto item : std::initializer_list<std::tuple<const char*, const char*, ModListSource*, const char*, bool>> {
        ...
        auto btn = CCMenuItemSpriteExtra::create(
            NoahhTabSprite::create(std::get<0>(item), std::get<1>(item), 100, std::get<4>(item)),
    ...
    auto smallSizeBtn = CCMenuItemSpriteExtra::create(
        NoahhSquareSprite::createWithSpriteFrameName("GJ_smallModeIcon_001.png"),
    ...
    auto bigSizeBtn = CCMenuItemSpriteExtra::create(
        NoahhSquareSprite::createWithSpriteFrameName("GJ_extendedIcon_001.png"),
        this, menu_selector(ModsLayer::onDisplay)
    );
    ...
    auto gridBtn = CCMenuItemSpriteExtra::create(
        NoahhSquareSprite::createWithSpriteFrameName("grid-view.png"_spr),
        this, menu_selector(ModsLayer::onDisplay)
    );
    ...
    //     NoahhSquareSprite::createWithSpriteFrameName("search.png"_spr, &m_showSearch),
...
void ModsLayer::gotoTab(ModListSource* src, bool searchingDev) {
    ...
    for (auto tab : m_tabs) {
        ...
        static_cast<NoahhTabSprite*>(tab->getNormalImage())->select(selected);
...
void ModsLayer::updateState() {
    ...
    for (auto btn : m_displayBtns) {
        static_cast<NoahhSquareSprite*>(btn->getNormalImage())->setState(
...
void ModsLayer::onTheme(CCObject*) {
    auto old = Mod::get()->getSettingValue<bool>("enable-noahh-theme");
    createQuickPopup(
        "Switch Theme",
        fmt::format(
            "Do you want to switch the <cp>color scheme</c> of the Noahh menu "
            "to {}?",
            (old ? "<cy>GD-style colors</c>" : "<ca>Noahh-style colors</c>")
        ),
        "Cancel", "Switch",
        [old](auto*, bool btn2) {
            if (btn2) {
                Mod::get()->setSettingValue("enable-noahh-theme", !old);
...
void ModsLayer::installModFromFile() {
    if (!Mod::get()->setSavedValue("shown-manual-install-info", true)) {
        return FLAlertLayer::create(
            ...
            "You can <cg>manually install mods</c> by selecting their <cd>.noahh</c> files. "
