...
CCNode* createKeybindButton(Keybind const& keybind) {
    if (keybind.key >= CONTROLLER_A && keybind.key <= CONTROLLER_RTHUMBSTICK_RIGHT) {
        return createNoahhButton(keybind.createNode(), "");
    }
    return createNoahhButton(nullptr, keybind.toString(), true);
}
...
bool KeybindEditPopup::init(
    ...
    if (keybind.key == KEY_None) {
        auto addButton = CCMenuItemSpriteExtra::create(
            createNoahhButton("Add", true), this, menu_selector(KeybindEditPopup::onSet)
        );
        ...
        auto setButton = CCMenuItemSpriteExtra::create(
            createNoahhButton("Set", true), this, menu_selector(KeybindEditPopup::onSet)
        );
        bottomMenu->addChild(setButton);

        auto removeButton = CCMenuItemSpriteExtra::create(
            createNoahhButton("Remove", true, NoahhButtonSprite::Delete), this, menu_selector(KeybindEditPopup::onRemove)
        );
...
bool KeybindListPopup::init(
    ...
    if (!NoahhPopup::init(220.f, 250.f))
        return false;
    ...
    m_mainLayer->addChildAtPosition(createNoahhListBorders({ 190.f, 150.f }), Anchor::Center, ccp(-5, 5));

    auto bottomMenu = CCMenu::create();
    bottomMenu->setContentWidth(220.f);

    auto addButton = CCMenuItemSpriteExtra::create(
        createNoahhButton("Add", true), this, menu_selector(KeybindListPopup::onAdd)
    );
    bottomMenu->addChild(addButton);

    auto saveButton = CCMenuItemSpriteExtra::create(
        createNoahhButton("Save", true), this, menu_selector(KeybindListPopup::onSave)
    );
