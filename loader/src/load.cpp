...
void tryLogForwardCompat() {
    ...
    log::warn("+-----------------------------------------------------------------------------------------------+");
    log::warn("| Noahh is running in a newer version of GD than Noahh targets.                                 |");
    log::warn("| UI is going to be disabled, platform console is forced on and crashes can be more common.     |");
    log::warn("| However, if your game crashes, it is probably caused by an outdated mod and not Noahh itself. |");
    log::warn("+-----------------------------------------------------------------------------------------------+");
}
...
void tryShowForwardCompat() {
    ...
    //     "Noahh is running in a newer version of GD than Noahh targets.\n"
    //     "UI is going to be disabled, platform console is forced on and crashes can be more common.\n"
    //     "However, if your game crashes, it is probably caused by an outdated mod and not Noahh itself.",
    //     Severity::Warning
    // );
...
bool safeModeCheck() {
    ...
    auto choice = MessageBoxW(
        ...
        L"Do you want to activate Noahh Safe Mode? This disables loading any mods.",
        L"Attention",
...
int noahhEntry(void* platformData) {
    ...
    if (LoaderImpl::get()->getGameVersion().empty()) {
        log::info("Running {} {}{} on {}", Mod::get()->getName(), Mod::get()->getVersion(),
            forwardCompatSuffix, PlatformID::toString(NOAHH_PLATFORM_TARGET));
    }
    else {
        log::info("Running {} {} in Geometry Dash v{}{} on {}", Mod::get()->getName(),
            ...
            PlatformID::toString(NOAHH_PLATFORM_TARGET));
    }
    ...
    log::info("Setting up internal mod");
    {
        ...
        if (!internalSetupRes) {
            console::messageBox(
                ...
                "the internal mod and Noahh can not be loaded: " + internalSetupRes.unwrapErr()
            );
