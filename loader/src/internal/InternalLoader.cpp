...
InternalLoader* InternalLoader::get() {
    static auto g_noahh = new InternalLoader;
    return g_noahh;
}
...
void InternalLoader::queueInGDThread(std::function<void NOAHH_CALL()> func) {
...
void InternalLoader::setupPlatformConsole() {
    ...
    freopen(ghc::filesystem::path(utils::dirs::noahhRoot() / "noahh_log.txt").string().c_str(),"w",stdout);
    InternalLoader::
    m_platformConsoleReady = true;
}
