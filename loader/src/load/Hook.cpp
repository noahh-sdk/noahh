...
#include <Noahh/hook-core/Hook.hpp>
...
NOAHH_STATIC_VAR(std::vector<hook_info>, internalHooks);
NOAHH_STATIC_VAR(bool, readyToHook);
...
Result<> Mod::disableHook(Hook* hook) {
    if (hook->isEnabled()) {
        if (noahh::core::hook::remove(hook->m_handle)) {
