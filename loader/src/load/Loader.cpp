...
#include <Noahh/utils/conststring.hpp>
#include <Noahh/utils/vector.hpp>
#include <Noahh/utils/map.hpp>
#include <Noahh/utils/types.hpp>
#include <mutex>
...
void Loader::createDirectories() {
    auto modDir = this->getNoahhDirectory() / NOAHH_MOD_DIRECTORY;
    auto logDir = this->getNoahhDirectory() / NOAHH_LOG_DIRECTORY;
    auto resDir = this->getNoahhDirectory() / NOAHH_RESOURCE_DIRECTORY;
    auto tempDir = this->getNoahhDirectory() / NOAHH_TEMP_DIRECTORY;
...
void Loader::updateResourcePaths() {
    ...
    // add mods directory
    CCFileUtils::sharedFileUtils()->addSearchPath(
        (this->getNoahhDirectory() / NOAHH_MOD_DIRECTORY).string().c_str()
    );
}
...
size_t Loader::loadModsFromDirectory(
    ...
    for (auto const& entry : ghc::filesystem::directory_iterator(dir)) {
        ...
        // skip this entry if its extension is not .noahh 
        if (entry.path().extension() != NOAHH_MOD_EXTENSION) {
            continue;
        }
...
void Loader::queueInGDThread(std::function<void NOAHH_CALL()> func) {
...
ghc::filesystem::path Loader::getNoahhDirectory() const {
    return noahh::utils::dirs::noahhRoot() / NOAHH_DIRECTORY;
}

ghc::filesystem::path Loader::getNoahhSaveDirectory() const {
    return this->getSaveDirectory() / NOAHH_DIRECTORY;
}
