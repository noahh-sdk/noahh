...
#include <Noahh/utils/json_check.hpp>
...
Result<> Mod::createTempDir() {
    ...
    auto tempDir = Loader::get()->getNoahhDirectory() / NOAHH_TEMP_DIRECTORY;
...
Result<> Mod::uninstall() {
    ...
    if (!ghc::filesystem::remove(m_info.m_path)) {
        return Err<>(
            "Unable to delete mod's .noahh file! "
...
Result<ModInfo> ModInfo::createFromSchemaV010(nlohmann::json const& rawJson) {
    ...
    if (json.contains("binary")) {
        ...
        if (autoEnd && !string_utils::endsWith(info.m_binaryName, NOAHH_PLATFORM_EXTENSION)) {
            info.m_binaryName += NOAHH_PLATFORM_EXTENSION;
        }
    }
    skip_binary_check:
...
Result<ModInfo> ModInfo::create(nlohmann::json const& json) {
    ...
    if (json.contains("noahh") && json["noahh"].is_string()) {
        auto ver = json["noahh"];
    ...
    if (schema < Loader::s_supportedVersionMin) {
        return Err(
            ...
            schema.toString() + ") of Noahh (current: " + 
    ...
    if (schema > Loader::s_supportedVersionMax) {
        return Err(
            ...
            "). You need to update Noahh in order to use "
            "this mod."
        );
    }
    ...
    return Err(
        ...
        LOADER_VERSION_STR + ") of noahh. "
        "This is probably a bug; report it to "
        "the Noahh Development Team."
    );
}
...
Result<ModInfo> ModInfo::createFromNoahhFile(ghc::filesystem::path const& path) {
