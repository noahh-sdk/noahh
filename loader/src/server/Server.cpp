...
#define NOAHH_GD_VERSION_STR NOAHH_STR(NOAHH_GD_VERSION)
...
Result<ServerModVersion> ServerModVersion::parse(matjson::Value raw) {
    ...
    res.metadata.setNoahhVersion(root.needs("noahh").get<VersionInfo>());

    // Verify target GD version
    auto gd_obj = root.needs("gd");
    std::string gd = "0.000";
    if (gd_obj.hasNullable(NOAHH_PLATFORM_SHORT_IDENTIFIER)) {
        gd = gd_obj.hasNullable(NOAHH_PLATFORM_SHORT_IDENTIFIER). get<std::string>();
    }
...
Result<ServerModUpdateAllCheck> ServerModUpdateAllCheck::parse(matjson::Value json) {
    ...
    if (json.isArray()) {
        return Ok(ServerModUpdateAllCheck {
            .updates = NOAHH_UNWRAP(ServerModUpdate::parseList(std::move(json))),
            .deprecations = {},
        });
    }
    auto root = checkJson(std::move(json), "ServerModUpdateAllCheck");
    auto updates = NOAHH_UNWRAP(ServerModUpdate::parseList(root.needs("updates").takeJson()));
    auto deprecations = NOAHH_UNWRAP(ServerModDeprecation::parseList(root.needs("deprecations").takeJson()));
...
Result<ServerModMetadata> ServerModMetadata::parse(matjson::Value raw) {
    ...
    if (root.has("created_at")) {
        NOAHH_UNWRAP_INTO(res.createdAt, ServerDateTime::parse(root.has("created_at").get<std::string>()));
    }
    if (root.has("updated_at")) {
        NOAHH_UNWRAP_INTO(res.updatedAt, ServerDateTime::parse(root.has("updated_at").get<std::string>()));
    }
...
Result<ServerLoaderVersion> ServerLoaderVersion::parse(matjson::Value raw) {
    ...
    auto gd_obj = root.needs("gd");
    gd_obj.needs(NOAHH_PLATFORM_SHORT_IDENTIFIER).into(res.gameVersion);
...
std::string server::getServerAPIBaseURL() {
    return "https://api.noahh-sdk.org/v1";
}
...
std::string server::getServerUserAgent() {
    ...
    static const auto value = [] {
        ...
        return fmt::format("Noahh Loader (ver={};commit={};platform={};gd={})",
            Loader::get()->getVersion().toNonVString(),
            about::getLoaderCommitHash(),
            NOAHH_PLATFORM_SHORT_IDENTIFIER,
...
ServerFuture<ServerModsList> server::getMods(ModsQuery query, bool useCache) {
    ...
    req.param("gd", NOAHH_GD_VERSION_STR);
    req.param("noahh", Loader::get()->getVersion().toNonVString());
...
ServerFuture<ServerModUpdateAllCheck> server::batchedCheckUpdates(std::vector<std::string> const& batch) {
    ...
    auto req = web::WebRequest();
    req.userAgent(getServerUserAgent());
    req.param("platform", NOAHH_PLATFORM_SHORT_IDENTIFIER);
    req.param("gd", NOAHH_GD_VERSION_STR);
    req.param("noahh", Loader::get()->getVersion().toNonVString());
    if (Loader::get()->isPatchless())
        req.param("jitless", "true");
...
ServerFuture<ServerLoaderVersion> server::getLatestLoaderVersion(bool useCache) {
    ...
    auto response = co_await req.get(
        formatServerURL("/loader/versions/latest?gd={}&platform={}",
            Loader::get()->getGameVersion(),
            NOAHH_PLATFORM_SHORT_IDENTIFIER_NOARCH
        )
    );
