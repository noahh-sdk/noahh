...
struct matjson::Serialize<server::RecentlyUpdatedMod> {
    ...
    static noahh::Result<server::RecentlyUpdatedMod> fromJson(Value const& value) {
        return noahh::Ok(server::RecentlyUpdatedMod {
            .modID = NOAHH_UNWRAP(value["id"].asString()),
            .updateTime = asp::SystemTime::fromUnix(NOAHH_UNWRAP(value["time"].asUInt())),
        });
    }
};
