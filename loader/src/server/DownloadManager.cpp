...
public:
    ...
    void onFinished(web::WebResponse response, ServerModVersion version) {
        ...
        if (auto mod = Loader::get()->getInstalledMod(id)) {
            ...
            if (ec) {
                m_status = DownloadStatusError {
                    .details = fmt::format("Unable to delete existing .noahh package (code {})", ec),
                };
                return;
            }
            // Mark mod as updated
        ...
        auto noahhPath = dirs::getModsDir() / (m_id + ".noahh");
        auto data = std::move(response).data();
        auto ok = file::writeBinary(noahhPath, data);
        if (!ok) {
            m_status = DownloadStatusError {
        ...
        auto metadata = ModMetadata::createFromNoahhFile(noahhPath);
