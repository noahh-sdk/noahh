...
class NoahhSettingsInfoSetting : public SettingV3 {
public:
    static Result<std::shared_ptr<SettingV3>> parse(std::string key, std::string modID, matjson::Value const& json) {
        auto res = std::make_shared<NoahhSettingsInfoSetting>();
        auto root = checkJson(json, "NoahhSettingsInfoSetting");
...
class NoahhSettingsInfoSettingNode : public SettingNodeV3 {
protected:
    bool init(std::shared_ptr<NoahhSettingsInfoSetting> setting, float width) {
        ...
        auto infoLabel = TextArea::create(
            "These are settings for the <co>Noahh Mod Loader</c>, NOT for mods. "
...
public:
    static NoahhSettingsInfoSettingNode* create(std::shared_ptr<NoahhSettingsInfoSetting> setting, float width) {
        auto ret = new NoahhSettingsInfoSettingNode();
...
SettingNodeV3* NoahhSettingsInfoSetting::createNode(float width) {
    return NoahhSettingsInfoSettingNode::create(
        std::static_pointer_cast<NoahhSettingsInfoSetting>(shared_from_this()),
        width
    );
}
...
protected:
    bool init(std::shared_ptr<CopyButtonSetting> setting, float width) {
        ...
        auto folderSpr = createNoahhButton("Open Mods Folder");
        ...
        auto copySpr = createNoahhButton("Copy Mods List");
        copySpr->setScale(.5f);
        ...
        auto installFromFileSpr = createNoahhButton("Install From File");
...
$on_mod(Loaded) {
    ...
    (void)Mod::get()->registerCustomSettingType("settings-info", &NoahhSettingsInfoSetting::parse);
}
