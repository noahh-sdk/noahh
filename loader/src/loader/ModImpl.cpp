...
Result<> Mod::Impl::setup() {
    ...
    NOAHH_UNWRAP(this->createTempDir().mapErr([](auto const& err) {
    ...
    if (!m_resourcesLoaded && !this->isInternal()) {
        ...
        auto const binaryPlatformId = PlatformID::toShortString(NOAHH_PLATFORM_TARGET NOAHH_MACOS(, true));
...
bool Mod::Impl::isInternal() const {
    return m_metadata.getID() == "noahh.loader";
}
...
Result<> Mod::Impl::loadData() {
    ...
    if (std::filesystem::exists(settingPath)) {
        NOAHH_UNWRAP_INTO(auto json, utils::file::readJson(settingPath));
    ...
    if (std::filesystem::exists(savedPath)) {
        NOAHH_UNWRAP_INTO(auto data, utils::file::readString(savedPath));
        m_saved = NOAHH_UNWRAP(matjson::parse(data).mapErr([](auto&& err) {
    ...
    if (!std::filesystem::exists(this->getBinaryPath())) {
        ...
        return Err(
            fmt::format(
                ...
                "This mod doesn't support this platform" NOAHH_WINDOWS(" or something deleted it (like an antivirus)") ".",
                m_metadata.getID()
            )
        );
    }
...
Result<> Mod::Impl::uninstall(bool deleteSaveData) {
    ...
    if (ec) {
        return Err(
            "Unable to delete mod's .noahh file: " + ec.message()
        );
    }
...
static Result<ModMetadata> getModImplInfo() {
    auto json = NOAHH_UNWRAP(matjson::parse(about::getLoaderModJson()).mapErr([](auto&& err) {
    ...
    if (!infoRes) {
        console::messageBox(
            ...
            "Unable to create internal mod info: \"" + infoRes.unwrapErr() +
                ...
                "contact Noahh developers immediately!"
        );
        mod = new Mod(ModMetadata("noahh.loader"));
    }
    else {
...
Result<> Loader::Impl::setupInternalMod() {
    NOAHH_UNWRAP(Mod::get()->m_impl->setup());
    auto resourcesDir = dirs::getNoahhResourcesDir() / Mod::get()->getID();
    NOAHH_UNWRAP(ModMetadataImpl::getImpl(ModImpl::get()->m_metadata).addSpecialFiles(resourcesDir));
    return Ok();
}
