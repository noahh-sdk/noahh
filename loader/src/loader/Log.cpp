...
#include <Noahh/loader/Types.hpp>
...
std::string noahh::format_as(Mod* mod) {
    if (mod) {
...
std::string noahh::format_as(CCObject const* obj) {
    if (obj) {
...
std::string noahh::format_as(CCNode* obj) {
    if (obj) {
        return fmt::format(
            "{{ {}, {}, ({}) }}",
            getObjectName(obj),
...
std::string noahh::format_as(CCArray* arr) {
    if (arr && arr->count()) {
...
void Logger::setup() {
    ...
    auto logDir = dirs::getNoahhLogDir();
    ...
    if (m_usingThread) {
        ...
        m_logThread.setName("Noahh Log Worker");
    }
}
...
std::string noahh::log::generateLogName() {
    auto now = asp::SystemTime::now();
    return fmt::format("Noahh {:%F %H.%M.%S}.log", asp::localtime(now.to_time_t()));
}
