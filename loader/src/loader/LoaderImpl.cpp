...
#include <Noahh/utils/string.hpp>
...
comm::EventCenter* noahh::comm::EventCenter::get() {
...
void Loader::Impl::createDirectories() {
    ...
    (void) utils::file::createDirectoryAll(dirs::getNoahhResourcesDir());
    (void) utils::file::createDirectoryAll(dirs::getModConfigDir());
    (void) utils::file::createDirectoryAll(dirs::getModsDir());
    (void) utils::file::createDirectoryAll(dirs::getNoahhLogDir());
...
void Loader::Impl::removeDirectories() {
    ...
    // clean up of stale data from Noahh v2
    if(std::filesystem::exists(dirs::getNoahhDir() / "index")) {
        async::runtime().spawnBlocking<void>([] {
            std::error_code ec;
            std::filesystem::remove_all(dirs::getNoahhDir() / "index", ec);
        });
    }
}
...
void Loader::Impl::addSearchPaths() {
    ...
    CCFileUtils::get()->addPriorityPath(utils::string::pathToString(dirs::getNoahhResourcesDir()).c_str());
...
void Loader::Impl::updateModResources(Mod* mod) {
    if (!mod->isInternal()) {
        // noahh.loader resource is stored somewhere else, which is already added anyway
    ...
    for (auto const& dir : m_modSearchDirectories) {
        ...
        for (auto const& entry : std::filesystem::directory_iterator(dir)) {
            if (!std::filesystem::is_regular_file(entry) ||
                entry.path().extension() != NOAHH_MOD_EXTENSION)
                continue;

            log::debug("Found {}", entry.path().filename());
            log::NestScope nest;

            auto modMetadata = ModMetadata::createFromNoahhFile(entry.path());
...
void Loader::Impl::loadModGraph(Mod* node, bool early) {
    ...
    if (!node->getMetadata().checkGameVersion() || !node->getMetadata().checkNoahhVersion()) {
        return;
    }
    ...
    auto unzipFunction = [this, node]() {
        log::debug("Unzipping .noahh file");
        auto res = this->unzipNoahhFile(node->getMetadata());
        return res;
    };
...
void Loader::Impl::findProblems() {
    for (auto const& [id, mod] : m_mods) {
        ...
        auto noahhVerRes = mod->getMetadata().checkNoahhVersion();
        if (!noahhVerRes) {
            this->addProblem({ LoadProblem::Type::Outdated, mod, noahhVerRes.unwrapErr() });
            log::error("{}", noahhVerRes.unwrapErr());
            continue;
        }

        if (mod->getMetadata().hasErrors()) {
            auto message = ranges::join(mod->getMetadata().getErrors(), ", ");
            this->addProblem({ LoadProblem::Type::InvalidNoahhFile, mod, message });
...
Result<> Loader::Impl::unzipNoahhFile(ModMetadata metadata) {
    // Unzip .noahh file into temp dir
    ...
    if (!res) {
        log::warn("Failed to write modified date of noahh zip, will try to unzip next launch: {}", res.unwrapErr());
    }
...
Result<> Loader::Impl::extractBinary(ModMetadata metadata) {
    ...
    // Extract the binary from the .noahh file
    NOAHH_UNWRAP_INTO(auto unzip, file::Unzip::create(metadata.getPath()));
    if (!unzip.hasEntry(metadata.getBinaryName())) {
        return Err(
            fmt::format("Unable to find platform binary under the name \"{}\"", metadata.getBinaryName())
    ...
    // TODO: enable in 4.7.0
    // NOAHH_UNWRAP(unzip.extractTo(metadata.getBinaryName(), dirs::getModBinariesDir() / metadata.getBinaryName()));
    NOAHH_UNWRAP(unzip.extractTo(metadata.getBinaryName(), dirs::getModRuntimeDir() / "binaries" / metadata.getBinaryName()));
...
Result<tulip::hook::HandlerHandle> Loader::Impl::getOrCreateHandler(void* address, tulip::hook::HandlerMetadata const& metadata) {
    ...
    NOAHH_UNWRAP_INTO(handle, tulip::hook::createHandler(address, metadata));
...
Result<> Loader::Impl::removeHandlerIfNeeded(void* address) {
    ...
    if (m_handlerHandles[address].second == 0) {
        NOAHH_UNWRAP(tulip::hook::removeHandler(handle));
    }
    return Ok();
}
...
void Loader::Impl::installModManuallyFromFile(std::filesystem::path const& path, noahh::Function<void()> after) {
    auto meta = ModMetadata::createFromNoahhFile(path);
    if (meta.hasErrors()) {
        ...
        FLAlertLayer::create(
            "Invalid File",
            fmt::format(
                "The path <cy>'{}'</c> is not a valid Noahh mod!\n"
    ...
    auto doInstallModFromFile = [this, path, meta, after = std::move(after)]() mutable {
        ...
        auto installTo = dirs::getModsDir() / fmt::format("{}.noahh", meta.getID());
        size_t counter = 0;
        while (std::filesystem::exists(installTo, ec) && counter < MAX_ATTEMPTS) {
            installTo = dirs::getModsDir() / fmt::format("{}-{}.noahh", meta.getID(), counter);
            counter += 1;
        }
