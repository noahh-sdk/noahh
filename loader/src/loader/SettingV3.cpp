...
namespace enable_if_parsing {
    ...
    struct RequireAll final : public Component {
        ...
        Result<> checkSemantics() const override {
            for (auto& comp : components) {
                NOAHH_UNWRAP(comp->checkSemantics());
            }
            return Ok();
        }
        bool shouldEnableSetting(std::string_view defaultModID) const override {
        ...
        std::string shouldEnableReason(std::string_view defaultModID) const override {
            ...
            return "If you see this, Noahh is broken";
        }
    };
    ...
    private:
        ...
        Result<std::unique_ptr<Component>> nextComponent() {
            NOAHH_UNWRAP_INTO(auto maybeWord, this->nextWord());
            if (!maybeWord) {
            ...
            if (word == "(") {
                NOAHH_UNWRAP_INTO(auto op, this->next());
                NOAHH_UNWRAP_INTO(auto maybeClosing, this->nextWord());
                if (!maybeClosing) {
        ...
        Result<std::unique_ptr<Component>> nextUnOp() {
            ...
            NOAHH_UNWRAP_INTO(auto comp, this->nextComponent());
            if (op.empty()) {
            ...
            switch (hash(op)) {
                ...
                default: {
                    return Err(
                        ...
                        "REPORT THIS BUG TO NOAHH DEVELOPERS",
                        op
                    );
                } break;
            }
        ...
        Result<std::unique_ptr<Component>> nextBiOp() {
            NOAHH_UNWRAP_INTO(auto first, this->nextUnOp());
            std::string firstOp;
            std::vector<std::unique_ptr<Component>> components;
            while (auto peek = this->peekWord()) {
                ...
                NOAHH_UNWRAP_INTO(auto word, this->nextWord());
                auto op = *word;
                if (firstOp.empty()) {
                    firstOp = op;
                }
                ...
                NOAHH_UNWRAP_INTO(auto comp, this->nextUnOp());
    ...
    public:
        static Result<std::unique_ptr<Component>> parse(std::string str, std::string defaultModID) {
            ...
            NOAHH_UNWRAP_INTO(auto comp, ret.next());
            NOAHH_UNWRAP_INTO(auto shouldBeEOF, ret.nextWord());
            if (shouldBeEOF) {
                return Err(
...
SettingV3::SettingV3() : m_impl(std::make_shared<NoahhImpl>()) {}
...
void SettingV3::parseEnableIf(JsonExpectedValue& json) {
    json.has("enable-if")
        .mustBe<std::string>("a valid \"enable-if\" scheme", [this](std::string str) -> Result<> {
            NOAHH_UNWRAP_INTO(auto tree, enable_if_parsing::Parser::parse(std::move(str), m_impl->modID));
            NOAHH_UNWRAP(tree->checkSemantics());
...
void KeybindSettingV3::parseBaseProperties(std::string key, std::string modID, JsonExpectedValue& json) {
    ...
    if (root.isObject() && root.has(NOAHH_PLATFORM_SHORT_IDENTIFIER_NOARCH)) {
        auto defaultValue = root.needs(NOAHH_PLATFORM_SHORT_IDENTIFIER_NOARCH);
