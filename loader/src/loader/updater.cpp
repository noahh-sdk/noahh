...
namespace {
    inline std::string formatDownloadUrl(std::string_view tag) {
        return fmt::format("https://github.com/noahh-sdk/noahh/releases/download/{0}/noahh-{0}-{1}.zip", tag, NOAHH_PLATFORM_SHORT_IDENTIFIER_NOARCH);
    }

    inline std::string formatResourcesUrl(std::string_view tag) {
        return fmt::format("https://github.com/noahh-sdk/noahh/releases/download/{}/resources.zip", tag);
    }
}
...
void updater::downloadLoaderUpdate(std::string url) {
    ...
    holder.spawn(
        ...
        [](web::WebResponse response) {
            ...
            auto targetDir = dirs::getNoahhDir() / "update";
    ...
    async::spawn(
        ...
        [](Result<server::ServerLoaderVersion, server::ServerError> res) {
            if (res.ok()) {
                ...
                log::info("Latest Noahh version is {}", ver.toVString());
