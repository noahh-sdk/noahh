...
#include <Noahh/utils/random.hpp>
#include <about.hpp>
#include <matjson.hpp>
#include <utility>
#include <clocale>
...
ModMetadata ModMetadata::Impl::parse(ModJson const& rawJson, std::optional<std::string_view> guessedID) {
    ...
    root.needs("noahh").into(impl->m_noahhVersion);

    if (auto gd = root.needs("gd")) {
        gd.assertIsObject();
        if (gd.has(NOAHH_PLATFORM_SHORT_IDENTIFIER_NOARCH)) {
            gd.needs(NOAHH_PLATFORM_SHORT_IDENTIFIER_NOARCH)
    ...
    if (info.getID() != "noahh.loader") {
        Dependency dep;
        dep.setID("noahh.loader");
        ...
        for (auto& [id, dep] : deps.properties()) {
            ...
            if (dep.isObject()) {
                ...
                for (auto& plat : dep.has("platforms").items()) {
                    if (PlatformID::coveredBy(plat.get<std::string>(), NOAHH_PLATFORM_TARGET)) {
                        onThisPlatform = true;
                    }
                }
                if (!onThisPlatform) {
    ...
    impl->m_binaryName = impl->m_id + NOAHH_PLATFORM_EXTENSION;
...
Result<> ModMetadata::Impl::addSpecialFiles(file::Unzip& unzip) {
    ...
    for (auto& [file, target] : this->getSpecialFiles()) {
        if (unzip.hasEntry(file)) {
            ...
            NOAHH_UNWRAP_INTO(auto data, unzip.extract(fileStr).mapErr([&](auto const& err) {
...
VersionInfo ModMetadata::getNoahhVersion() const {
    return m_impl->m_noahhVersion;
}
...
Result<> ModMetadata::checkGameVersion() const {
    if (!m_impl->m_gdVersion.empty() && m_impl->m_gdVersion != "*") {
        ...
        if (LoaderImpl::get()->isForwardCompatMode()) {
            // this means current gd version is > NOAHH_GD_VERSION
            if (modTargetVer <= NOAHH_GD_VERSION) {
                return Err(fmt::format("This mod doesn't support this version of Geometry Dash ({})", ver));
            }
        } else if (ver != NOAHH_STR(NOAHH_GD_VERSION)) {
            // we are not in forward compat mode, so NOAHH_GD_VERSION is the current gd version
            return Err(
                fmt::format(
                    "This mod ({}) was created for a different version of Geometry Dash ({}). You currently have version {}.",
                    m_impl->m_id, ver, NOAHH_STR(NOAHH_GD_VERSION)
                )
            );
        }
    }
...
Result<> ModMetadata::checkNoahhVersion() const {
    if (!LoaderImpl::get()->isModVersionSupported(m_impl->m_noahhVersion)) {
        auto current = LoaderImpl::get()->getVersion();
        if (m_impl->m_noahhVersion > current) {
            return Err(
                "This mod ({}) was made for a newer version of Noahh ({}). You currently have version {}.",
                m_impl->m_id, m_impl->m_noahhVersion, current
            );
        }
        else {
            return Err(
                "This mod ({}) was made for an older version of Noahh ({}). You currently have version {}.",
                m_impl->m_id, m_impl->m_noahhVersion, current
            );
        }
    }
...
Result<> ModMetadata::checkTargetVersions() const {
    NOAHH_UNWRAP(this->checkGameVersion());
    NOAHH_UNWRAP(this->checkNoahhVersion());
    return Ok();
}
...
void ModMetadata::setNoahhVersion(VersionInfo value) {
    m_impl->m_noahhVersion = std::move(value);
}
...
ModMetadata ModMetadata::createFromNoahhFile(std::filesystem::path const& path) {
    // result code looks weird when you don't want to return a result

    // Try guess ID from filename (since usually Noahh mods are named `mod.id.noahh`)
    ...
    if (!unzip.hasEntry("mod.json")) {
        return Impl::createInvalidMetadata(path, "Noahh package is missing \"mod.json\"", guessedID);
    }
