...
private:
    ...
    OldCKSaveData()
      : m_data(file::readJson(
            dirs::getModsSaveDir() / "noahh.custom-keybinds" / "saved.json"
        ).unwrapOrDefault())
    {}
    ...
    enumKeyCodes convertWithDevice(std::string_view device, int code) {
        switch (hash(device)) {
            ...
            case hash("noahh.custom-keybinds/controller"):
            case hash("noahh.custom-keybinds/keyboard"): {
                return static_cast<enumKeyCodes>(code);
            } break;

            // This is a special enum in CK (with two members lol)
            case hash("noahh.custom-keybinds/mouse"): {
                switch (code) {
                    default:
...
Result<> ModSettingsManager::registerCustomSettingType(std::string_view type, SettingGenerator generator) {
    NOAHH_UNWRAP(SharedSettingTypesPool::get().add(m_impl->modID, type, std::move(generator)));
    m_impl->createSettings();
