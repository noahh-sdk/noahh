...
#include <Noahh/modify/CCLayer.hpp>
...
struct CustomLoadingLayer : Modify<CustomLoadingLayer, LoadingLayer> {
    ...
    static void onModify(auto& self) {
        if (!self.setHookPriority("LoadingLayer::init", noahh::node_ids::NOAHH_ID_PRIORITY)) {
            log::warn("Failed to set LoadingLayer::init hook priority, node IDs may not work properly");
        }
        NOAHH_FORWARD_COMPAT_DISABLE_HOOKS_INNER("Switching to fallback custom loading layer")
    }
    ...
    void updateLoadedModsLabel() {
        ...
        auto str = fmt::format("Noahh: Loaded {}/{} mods", count, m_fields->m_totalMods);
    ...
    bool init(bool fromReload) {
        ...
        m_fields->m_smallLabel->setID("noahh-small-label");
        ...
        m_fields->m_smallLabel2->setID("noahh-small-label-2");
    ...
    void setupLoaderResources() {
        ...
        this->setSmallText("Verifying Noahh Resources");
        // verify loader resources
        Loader::get()->queueInMainThread([&]() {
            if (!updater::verifyLoaderResources()) {
                log::debug("Downloading Loader Resources");
                this->setSmallText("Downloading Noahh Resources");
                ...
                this->setSmallText("Loading Noahh Resources");
        ...
        std::visit(makeVisitor {
            [&](updater::UpdateProgress const& progress) {
                this->setSmallText(fmt::format(
                    "Downloading Noahh Resources: {}%", progress.first
                ));
            },
            [&](updater::UpdateFinished) {
                log::debug("Downloaded Loader Resources");
                this->setSmallText("Downloaded Noahh Resources");
                this->continueLoadAssets();
            },
            [&](updater::UpdateFailed const& error) {
                ...
                console::messageBox(
                    ...
                    "https://github.com/noahh-sdk/noahh/releases/latest.\n"
                    "The game will be loaded as normal, but please be aware "
                    "that it is very likely to crash. "
                );
                this->setSmallText("Failed to Download Noahh Resources");
                this->continueLoadAssets();
            }
        }, status);
    ...
    int getCurrentStep() {
        return m_fields->m_noahhLoadStep + m_loadStep + getLoadedMods();
    }
    ...
    // hook
    void loadAssets() {
        switch (m_fields->m_noahhLoadStep) {
        case 0:
...
struct FallbackCustomLoadingLayer : Modify<FallbackCustomLoadingLayer, CCLayer> {
    static void onModify(auto& self) {
        NOAHH_FORWARD_COMPAT_ENABLE_HOOKS_INNER("")
        else if (!self.setHookPriority("CCLayer::init", noahh::node_ids::NOAHH_ID_PRIORITY)) {
    ...
    bool init() {
        ...
        auto label = CCLabelBMFont::create(
            "Loading Noahh without UI, see console for details.",
            "goldFont.fnt"
        );
