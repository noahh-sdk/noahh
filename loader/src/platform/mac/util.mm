...
NOAHH_DLL arc::Future<Result<std::optional<std::filesystem::path>>> file::pick(file::PickMode mode, file::FilePickOptions options) {
...
NOAHH_DLL arc::Future<Result<std::vector<std::filesystem::path>>> file::pickMany(file::FilePickOptions options) {
    auto [tx, rx] = arc::oneshot::channel<FileResult>();
    __block auto sender = std::move(tx);
...
void noahh::utils::game::launchLoaderUninstaller(bool deleteSaveData) {
    log::error("Launching Noahh uninstaller is not supported on macOS");
}
...
float noahh::utils::getDisplayFactor() {
