...
void console::openIfClosed() {
    ...
    auto scriptContent = fmt::format(R"(
        #!/bin/sh
        echo -n -e "\033]0;Noahh Console {}\007"
        tail -f {} &
        trap "" SIGINT
        lsof -p {} +r 1 &>/dev/null
        pkill -P $$
        osascript -e 'tell application "Terminal"
            close (every window whose name contains "Noahh Console {}")
...
CFDataRef msgPortCallback(CFMessagePortRef port, SInt32 messageID, CFDataRef data, void* info) {
    ...
    std::string reply = noahh::ipc::processRaw(port, cdata).dump();

    return CFDataCreate(NULL, (UInt8 const*)reply.data(), reply.size());
}

void noahh::ipc::setup() {
    std::thread([]() {
        thread::setName("Noahh Main IPC");
...
std::string Loader::Impl::getGameVersion() {
    ...
    if (gameVersionIsAmbiguous(versionStr)) {
        static std::string manualVersionStr = []() -> std::string {
            ...
            // in this situation, noahh (and any mods) won't load, so it's not really a big deal
            return uuidStr;
        }();
