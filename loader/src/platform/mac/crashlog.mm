...
static Mod* modFromAddress(void const* addr) {
    ...
    auto noahhPath = dirs::getGameDir() / "Frameworks" / "Noahh.dylib";
    if (std::filesystem::equivalent(imagePath, noahhPath)) {
        return Mod::get();
    }
...
static std::string getStacktrace() {
    ...
    auto gameDir = noahh::utils::string::pathToString(dirs::getGameDir());
    ...
    for (int i = 1; i < s_backtraceSize; ++i) {
        ...
        binary = noahh::utils::string::trim(binary.substr(0, cutoff));
        ...
        if (!line.empty()) {
            ...
            stream >> offset;
            function = noahh::utils::string::trim(function.substr(0, cutoff));
...
std::filesystem::path crashlog::getCrashLogDirectory() {
    return dirs::getNoahhDir() / "crashlogs";
}
