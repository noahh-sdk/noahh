...
static std::string getImageName(struct dyld_image_info const* image) {
    ...
    if (
        imageName.ends_with("Noahh.ios.dylib")
        || imageName.ends_with("GeometryJump")
        || imageName.find("game/noahh/unzipped") != std::string::npos
    ) {
...
static Mod* modFromAddress(void const* addr) {
    ...
    auto noahhPath = dirs::getNoahhDir() / "Noahh.ios.dylib";
    if (imagePath.filename() == noahhPath.filename()) {
        return Mod::get();
    }
