...
void console::openIfClosed() {
    ...
    freopen(utils::string::pathToString(dirs::getNoahhDir() / "noahh_log.txt").c_str(), "w", stdout);
    s_isOpen = true;
}
...
/**
 * iOS Jitless is implemented with Noahh 4.6.0, so we kinda did a major change
