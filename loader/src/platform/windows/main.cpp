...
void updateNoahh() {
    ...
    const auto noahhDir = dirs::getNoahhDir();
    const auto updatesDir = noahhDir / "update";

    bool bootstrapperExists = std::filesystem::exists(workingDir / "NoahhBootstrapper.dll");
    bool updatesDirExists = std::filesystem::exists(noahhDir) && std::filesystem::exists(updatesDir);
    ...
    if (std::filesystem::exists(updatesDir) &&
        std::filesystem::exists(updatesDir / "NoahhUpdater.exe"))
        std::filesystem::rename(updatesDir / "NoahhUpdater.exe", workingDir / "NoahhUpdater.exe");
...
void patchDelayLoad() {
    ...
    auto address = noahh::cast::reference_cast<uintptr_t>(&cocos2d::CCNode::convertToNodeSpace);
...
bool cleanModeCheck() {
    if (
        ...
        auto choice = MessageBoxW(
            ...
            L"Do you want to open Geometry Dash without Noahh?",
            L"Attention",
...
int WINAPI gdMainHook(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow) {
    ...
    if (!cleanModeCheck()) {
        if (versionToTimestamp(NOAHH_STR(NOAHH_GD_VERSION)) > gdTimestamp) {
            console::messageBox(
            ...
            // TODO: should noahh FreeLibrary itself here?
        } else {
            patchDelayLoad();

            int exitCode = noahhEntry(hInstance);
            if (exitCode != 0)
                return exitCode;
        }
    }
...
#if 0
#define MSG_BOX_DEBUG(...) MessageBoxW(NULL, utils::string::utf8ToWide(std::format(NOAHH_STR(__LINE__) " - " __VA_ARGS__)).c_str(), L"Noahh", 0)
#else
#define MSG_BOX_DEBUG(...)
#endif

std::string loadNoahh() {
    auto process = GetCurrentProcess();
    auto dosHeader = reinterpret_cast<IMAGE_DOS_HEADER*>(noahh::base::get());
    auto ntHeader = reinterpret_cast<PIMAGE_NT_HEADERS>(noahh::base::get() + dosHeader->e_lfanew);

    gdTimestamp = ntHeader->FileHeader.TimeDateStamp;

    constexpr size_t trampolineSize = NOAHH_WINDOWS64(32) NOAHH_WINDOWS32(12);
    mainTrampolineAddr = VirtualAlloc(
        nullptr, trampolineSize,
        MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE
    );

    const uintptr_t entryAddr = noahh::base::get() + ntHeader->OptionalHeader.AddressOfEntryPoint;
    ...
    MSG_BOX_DEBUG("Entry address: {:x}, {:x}", entryAddr, entryAddr - noahh::base::get());
    ...
    constexpr ptrdiff_t mainCallJmpOffset = NOAHH_WINDOWS64(13) NOAHH_WINDOWS32(5);
    ...
    // 32 bit:
    // 6a 00                  push 0
    // 68 00 00 40 00         push noahh::base::get()
    // e8 ...                 call ...
    ...
    constexpr uint64_t mainSearchBytes = NOAHH_WINDOWS64(0xd233c08b4ccb8b44) NOAHH_WINDOWS32(0x004000006a006800);
    constexpr ptrdiff_t mainSearchCallOffset = NOAHH_WINDOWS64(15) NOAHH_WINDOWS32(7);

#ifdef NOAHH_IS_WINDOWS32
    mainSearchBytes |= static_cast<uint64_t>(noahh::base::get()) << 24;
#endif
    ...
    MSG_BOX_DEBUG("Searching from {:x}, aka {:x}", preWinMainAddr, preWinMainAddr - noahh::base::get());
    ...
    if (patchAddr == 0)
        return "Noahh could not find the main function, not loading Noahh.";
...
#define JMP_BYTES(from, to) \
    ...
    MSG_BOX_DEBUG("found the main address {:x}", patchAddr - noahh::base::get());

    DWORD oldProtect;
    if (!VirtualProtectEx(process, reinterpret_cast<void*>(patchAddr), patchSize, PAGE_EXECUTE_READWRITE, &oldProtect))
        return "Noahh could not hook the main function, not loading Noahh.";
...
void earlyError(std::string message) {
    ...
    std::ofstream fout("_noahh_early_error.txt");
    fout << message;
    fout.close();
    console::messageBox("Unable to Load Noahh!", message);
}
...
BOOL WINAPI DllMain(HINSTANCE module, DWORD reason, LPVOID) {
    ...
    // if we find the old bootstrapper dll, don't load noahh, copy new updater and let it do the rest
    auto workingDir = dirs::getGameDir();
    std::error_code error;
    bool oldBootstrapperExists = std::filesystem::exists(workingDir / "NoahhBootstrapper.dll", error);
    if (error) {
        earlyError("There was an error checking whether the old NoahhBootstrapper.dll exists: " + error.message());
        return FALSE;
    }
    else if (oldBootstrapperExists)
        CreateThread(nullptr, 0, upgradeThread, nullptr, 0, nullptr);
    else if (auto error = loadNoahh(); !error.empty()) {
        earlyError(error);
        return TRUE;
    }
...
[[noreturn]] void NOAHH_DLL __cdecl throw_cpp_error_hook(int code) {
...
static void fixThrowCppErrorStub() {
    ...
    for (auto byte : noahh::toBytes(&throw_cpp_error_hook)) {
