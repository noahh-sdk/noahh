...
bool redirectStd(FILE* which, ZStringView name, const Severity sev) {
    auto pipeName = utils::string::utf8ToWide(fmt::format(R"(\\.\pipe\noahh-{}-{})", name, GetCurrentProcessId()));
...
void console::setup() {
    ...
    if (s_outHandle) {
        ...
        setupConsole(
            ...
            || noahh::utils::getEnvironmentVariable("NOAHH_FORCE_ENABLE_TERMINAL_COLORS") != "0" // prefer to use FORCE_COLOR=1
            || noahh::utils::getEnvironmentVariable("FORCE_COLOR") != "0"
        );
    }
