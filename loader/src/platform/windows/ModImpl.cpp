...
Result<> Mod::Impl::loadPlatformBinary() {
    ...
    if (load) {
        ...
        auto noahhImplicitEntry = findSymbolOrMangled<void(*)()>(load, "noahhImplicitEntry", "_noahhImplicitEntry@0");
        if (noahhImplicitEntry) {
            noahhImplicitEntry();
        }

        auto noahhCustomEntry = findSymbolOrMangled<void(*)()>(load, "noahhCustomEntry", "_noahhCustomEntry@0");
        if (noahhCustomEntry) {
            noahhCustomEntry();
        }
        return Ok();
