...
#include <Noahh/cocos/robtop/keyboard_dispatcher/CCKeyboardDispatcher.h>
...
LRESULT CALLBACK NoahhRawInputWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg != WM_INPUT) {
...
struct NoahhRawInput : Modify<NoahhRawInput, CCEGLView> {
    void pumpRawInput() {
    ...
    static void attemptHookRawInput() {
        ...
        g_originalRawInputProc = reinterpret_cast<WNDPROC>(SetWindowLongPtrW(
            ...
            reinterpret_cast<LONG_PTR>(NoahhRawInputWndProc)
        ));
    }
...
struct NoahhControllerInput : Modify<NoahhControllerInput, CCApplication> {
