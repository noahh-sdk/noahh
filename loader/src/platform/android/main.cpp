...
namespace {
    bool reportPlatformCapability(std::string id) {
        JniMethodInfo t;
        if (JniHelper::getStaticMethodInfo(t, "com/noahh/launcher/utils/NoahhUtils", "reportPlatformCapability", "(Ljava/lang/String;)Z")) {
...
extern "C" [[gnu::visibility("default")]] jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    auto updatePath = noahh::dirs::getGameDir() / "update";
    std::error_code ec;
    std::filesystem::remove_all(updatePath, ec);
    if (ec) {
        noahh::log::warn("Failed to remove update directory: {}", ec.message());
    }
