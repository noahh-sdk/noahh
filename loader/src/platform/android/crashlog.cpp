...
namespace {
    ...
    bool crashCallback(
        ...
        auto indicatorString = noahh::utils::string::pathToString(crashIndicatorPath);

        sys_open(indicatorString.c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0);

        // another memory allocation!
        noahh::log::error("Noahh crashed! Crash dump saved to {}", descriptor.path());
...
bool crashlog::setupPlatformHandler() {
    ...
    (void)noahh::utils::file::createDirectoryAll(logDirectory);

    google_breakpad::MinidumpDescriptor descriptor(noahh::utils::string::pathToString(logDirectory), crashdumpName());
...
bool crashlog::setupPlatformHandler() {
    ...
    if (JniHelper::getStaticMethodInfo(t, "com/noahh/launcher/utils/NoahhUtils", "getLogcatCrashBuffer", "()Ljava/lang/String;")) {
...
std::filesystem::path crashlog::getCrashLogDirectory() {
    return noahh::dirs::getNoahhDir() / "crashlogs";
}
