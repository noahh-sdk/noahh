...
void Index::updateIndexThread(bool force) {
    auto indexDir = Loader::get()->getNoahhDirectory() / "index";
    ...
    auto commit = fetchJSON(
        "https://api.github.com/repos/noahh-sdk/mods/commits"
    );
    if (!commit) {
    ...
    if (!json.is_array()) {
        return indexUpdateProgress(
            ...
            "Report this bug to the Noahh developers!"
        );
    }
    ...
    if (force || currentCommitSHA != upcomingCommitSHA) {
        ...
        auto gotZip = fetchFile(
            "https://github.com/noahh-sdk/mods/zipball/main",
            indexDir / "index.zip"
        );
        if (!gotZip) {
            return indexUpdateProgress(
...
void Index::updateIndexFromLocalCache() {
    m_items.clear();
    auto indexDir = Loader::get()->getNoahhDirectory() / "index" / "index";
...
std::vector<IndexItem> Index::getUninstalledItems() const {
    ...
    for (auto& item : m_items) {
        if (!Loader::get()->isModInstalled(item.m_info.m_id)) {
            if (item.m_download.count(NOAHH_PLATFORM_TARGET)) {
    ...
    for (auto& item : items) {
        if (!item.m_download.count(NOAHH_PLATFORM_TARGET)) {
            return Err(
                ...
                "current platform \"" NOAHH_PLATFORM_NAME "\" - Sorry! :("
            );
        }
        auto download = item.m_download.at(NOAHH_PLATFORM_TARGET);
        if (!download.m_url.size()) {
            return Err(
                "Download URL not set! Report this bug to "
                "the Noahh developers - this should not happen, ever."
            );
        }
