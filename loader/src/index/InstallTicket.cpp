...
void InstallTicket::install(std::string const& id) {
    ...
    if (notFound && notFound.value() == "Not Found") {
        ...
        return this->postProgress(
            ...
            "this to the Noahh development team."
        );
    }
    ...
    if (checksum != download.m_hash) {
        ...
        return this->postProgress(
            ...
            "report this to the Noahh development team."
        );
    }

    // move temp file to noahh directory
    try {
        auto modDir = Loader::get()->getNoahhDirectory() / "mods";
        ...
        if (!m_replaceFiles) {
            ...
            while (ghc::filesystem::exists(targetFile)) {
                targetFile = modDir /
                    (filename + std::to_string(number) + ".noahh");
                number++;
            }
        }
