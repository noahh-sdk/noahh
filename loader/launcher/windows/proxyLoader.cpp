...
static std::wstring getErrorString(DWORD error) {
    return L"Could not load Noahh! Error code: " + std::to_wstring(error);
}

static DWORD errorThread(LPVOID param) {
    constexpr wchar_t REDIST_ERROR[] = L"Could not load Noahh!\n"
    ...
    constexpr wchar_t ALT_REDIST_ERROR[] = L"Could not load Noahh!\n\n"
...
BOOL WINAPI DllMain(HINSTANCE module, DWORD reason, LPVOID _) {
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        // This is UB.
        if (LoadLibraryW(L"Noahh.dll") == NULL) {
