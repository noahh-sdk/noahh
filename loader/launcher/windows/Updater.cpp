...
std::filesystem::path noahhDir;
std::filesystem::path updatesDir;
std::filesystem::path resourcesDir;

void showError(std::wstring const& error) {
    MessageBoxW(nullptr, error.c_str(), L"Error Loading Noahh", MB_ICONERROR);
}
...
bool waitForFile(std::filesystem::path const& path) {
    ...
    if (hFile) {
        ...
        showError(L"Unable to update Noahh: " + filename.native() + L" is open by another process.");
        return false;
    }
    return true;
}
...
bool updateFile(std::string const& name) {
    ...
    if (error) {
        showError(L"Unable to update Noahh: Unable to move " + utf8ToWide(name), error);
        return false;
    }
    return true;
}
...
void removePath(std::filesystem::path const& path) {
    ...
    if (error) {
        if (path.has_filename())
            showError(L"Unable to update Noahh: Unable to remove " + path.filename().native(), error);
        else
            showError(L"Unable to update Noahh: Unable to remove " + path.native(), error);
        return;
    }
}
...
void updateResources() {
    ...
    if (error) {
        showError(L"Unable to update Noahh resources", error);
        return;
    }
...
int main(int argc, char* argv[]) {
    ...
    noahhDir = workingDir / "noahh";
    updatesDir = noahhDir / "update";
    resourcesDir = noahhDir / "resources";

    if (std::filesystem::exists(workingDir / "NoahhBootstrapper.dll"))
        removePath(workingDir / "NoahhBootstrapper.dll");

    if (std::filesystem::exists(noahhDir) && std::filesystem::exists(updatesDir)) {
        bool updateSuccess = true;
        updateSuccess &= updateFile("XInput1_4.dll");
        updateSuccess &= updateFile("Noahh.dll");
        updateSuccess &= updateFile("Noahh.pdb");
        updateResources();
