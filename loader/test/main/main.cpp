...
$on_mod(Loaded) {
    ...
    auto h1 = noahh::Dispatch<std::string>("noahh.test/test-garage-open").listen([](std::string const& str) {
        log::info("Received dispatched event: {}", str);
    });

    noahh::Dispatch<std::string>("noahh.test/test-garage-open").send("Hello from dispatch!");

    auto handle = Dispatch<int&>("test").listen([](int& val) {
        noahh::log::info("Received dispatched int: {}", val);
        val += 10;
        return val > 0;
    });
    
    auto handle2 = Dispatch<int>("test2").listen([](int val) {
        noahh::log::info("Received dispatched int2: {}", val);
    });

    auto handle3 = Dispatch<float>("test").listen([](float val) {
        noahh::log::info("Received dispatched float: {}", val);
    });

    auto handle4 = Dispatch<int&>("test").listen([](int& val) {
        noahh::log::info("Received dispatched int4: {}", val);
        val += 10;
    }, -5);

    auto handle5 = Dispatch<int&>("test").listen([](int& val) {
        noahh::log::info("Received dispatched int5: {}", val);
        val += 10;
    }, 5);

    int value = 5;
    noahh::log::info("Value before dispatch: {}", value);
    Dispatch<int&>("test").send(value);
    noahh::log::info("Value after dispatch: {}", value);
...
struct $modify(MenuLayer) {
    bool init() {
        ...
        if (NOAHH_UNWRAP_IF_OK(val, api::addNumbers(5, 6))) {
            log::info("5 + 6 = {}", val);
        }
        else {
        ...
        api::Test test;
        if (NOAHH_UNWRAP_IF_OK(val, test.addNumbers(5, 6))) {
            log::info("5 + 6 = {}", val);
        }
        else {
...
// Modify
#include <Noahh/modify/GJGarageLayer.hpp>
...
struct GJGarageLayerTest : Modify<GJGarageLayerTest, GJGarageLayer> {
    ...
    bool init() {
        ...
        MyDispatchEvent("noahh.test/test-garage-open").send(this);
    ...
    bool ccTouchBegan(CCTouch* touch, CCEvent* event) override {
        ...
        if (m_fields->counter % 2) {
            if (auto r = VMTHookManager::get().forceDisableFunction<
                ResolveC<GJGarageLayerTest2>::func(&GJGarageLayerTest2::ccTouchEnded)
            >(this); !r) {
                noahh::log::warn("forceDisableFunction failed: {}", r.unwrapErr());
            }
        }
        else {
            if (auto r = VMTHookManager::get().forceEnableFunction<
                ResolveC<GJGarageLayerTest2>::func(&GJGarageLayerTest2::ccTouchEnded)
            >(this); !r) {
                noahh::log::warn("forceEnableFunction failed: {}", r.unwrapErr());
            }
        }
        m_fields->counter++;
        return true;
