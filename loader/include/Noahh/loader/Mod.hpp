...
namespace noahh {
    ...
    protected:
        ...
        template <class = void>
        static inline NOAHH_HIDDEN Mod* sharedMod = nullptr;

        // used internally in noahh_implicit_load
        template <class = void>
        static inline NOAHH_HIDDEN void setSharedMod(Mod* mod) {
            sharedMod<> = mod;
        }

        friend void NOAHH_CALL ::noahh_implicit_load(Mod*);
    ...
    public:
         ...
         * `dependencies."noahh.custom-keybinds".settings.keybinds`
         ...
         * `openInfoPopup` from the `ui/NoahhUI.hpp` header to open the Mod's
         ...
         * `noahh::dirs::getModsSaveDir()`/{mod_id}/
         */
        std::filesystem::path getSaveDir() const;
        /**
         * Get the mod's config directory path
         * `noahh::dirs::getModConfigDir()`/{mod_id}/
         */
        std::filesystem::path getConfigDir(bool create = true) const;
        /**
         * Get the mod's persistent directory path
         * This directory is not deleted even when Noahh/mod is uninstalled
         * `noahh::dirs::getModPersistentDir()`/{mod_id}/
         */
         ...
         * [the setting docs](https://docs.noahh-sdk.org/mods/settings) for more
         ...
         * `mod-arg` for the mod `author.test` would be specified with `--noahh:author.test.mod-arg=value`.
         * @param name The argument name
        ...
        /**
         * Get the value of a [setting](https://docs.noahh-sdk.org/mods/settings).
         ...
         */
        template <class = void>
        static inline NOAHH_HIDDEN Mod* get() {
            if (!sharedMod<>) {
        ...
        Result<Hook*> hook(
            ...
            NOAHH_UNWRAP_INTO(auto ptr, this->claimHook(std::move(hook)));
            return Ok(ptr);
        }
        ...
        Result<Patch*> patch(void* address, ByteSpan data) {
            ...
            NOAHH_UNWRAP_INTO(auto ptr, this->claimPatch(std::move(patch)));
            return Ok(ptr);
        }
        ...
        /**
         * Delete the mod's .noahh package.
        ...
        /**
         * If this mod is built for an outdated GD or Noahh version, returns the
         * `LoadProblem` describing the situation. Otherwise `nullopt` if the
         * mod is made for the correct version of the game and Noahh
         */
...
namespace noahh::noahh_internal {
    // this impl relies on the NOAHH_MOD_ID macro set by cmake
    template <size_t N>
    struct StringConcatModIDSlash {
        static constexpr size_t extra = sizeof(NOAHH_MOD_ID);
        char buffer[extra + N]{};
        constexpr StringConcatModIDSlash(const char (&pp)[N]) {
            char id[] = NOAHH_MOD_ID;
...
template <noahh::noahh_internal::StringConcatModIDSlash Str>
...
// NOAHH_HIDDEN inline char const* operator""_spr(char const* str, size_t len) {
//     return noahh::Mod::get()->expandSpriteName({ str, len }).data();
// }

NOAHH_HIDDEN inline char const* operator""_spr(char const* str, size_t len) {
    noahh::log::error("NOAHH_MOD_ID not defined, _spr cannot be used");
    return nullptr;
}
 ...
 * @example
 * ```cpp
 * NOAHH_MOD_STATIC_PATCH(0x1234, "\x12\x34\x56\x78");
 * NOAHH_MOD_STATIC_PATCH(0x5678, {0x12, 0x34, 0x56, 0x78});
 * ```
 */
#define NOAHH_MOD_STATIC_PATCH(Offset_, ...) \
    noahh::doNotOptimize(noahh::utils::string::ConstexprString<>::toLiteral([](){ \
        noahh::utils::string::ConstexprString<> str2;            \
        str2.push(__VA_ARGS__);                                  \
        noahh::utils::string::ConstexprString<> str;             \
        str.push("[NOAHH_PATCH_SIZE]");                          \
        str.push(str2.size(), 16);                               \
        str.push("[NOAHH_PATCH_BYTES]");                         \
        str.push(str2);                                          \
        str.push("[NOAHH_PATCH_OFFSET]");                        \
        str.push(Offset_, 16);                                   \
        str.push("[NOAHH_PATCH_END]");                           \
        return str;                                              \
    }))
 ...
 * @example
 * ```cpp
 * auto res = NOAHH_MOD_STATIC_HOOK(0x1234, &myDetour, "MenuLayer::init");
 * ```
 */
#define NOAHH_MOD_STATIC_HOOK(Offset_, Detour_, ...) \
    (noahh::doNotOptimize(noahh::utils::string::ConstexprString<>::toLiteral([](){ \
        noahh::utils::string::ConstexprString<> str;               \
        str.push("[NOAHH_MODIFY_NAME]");                           \
        str.push(NOAHH_STR(__VA_ARGS__));                          \
        str.push("[NOAHH_MODIFY_OFFSET]");                         \
        str.push(Offset_, 16);                                     \
        str.push("[NOAHH_MODIFY_END]");                            \
        return str;                                                \
    })), noahh::Mod::get()->hook(                                  \
        reinterpret_cast<void*>(noahh::base::get() + Offset_),     \
        Detour_,                                                   \
        NOAHH_STR(__VA_ARGS__)))
