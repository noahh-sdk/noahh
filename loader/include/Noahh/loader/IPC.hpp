...
namespace noahh::ipc {
    ...
    constexpr char const* IPC_PIPE_NAME = R"(\\.\pipe\NoahhIPCPipe)";
    #endif

    #ifdef NOAHH_IS_MACOS
    constexpr char const* IPC_PORT_NAME = "NoahhIPCPipe";
    #endif

    class IPCFilter;

    // IPC (Inter-Process Communication) provides a way for Noahh mods to talk
    ...
    // example, an external application can query what mods are loaded in Noahh
    // by sending the `list-mods` message to `noahh.loader`.
