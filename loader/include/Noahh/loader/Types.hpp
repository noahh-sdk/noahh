...
namespace noahh {
	...
	/**
	 * Default Noahh load method for C++ 
	 * mods: The mod receive a pointer to 
	 * its allocated Mod instance. Return 
	 * true on a succesful load, 
	 * return false on error.
	 */
	typedef bool(NOAHH_CALL* noahh_load)(Mod*);
	typedef void(NOAHH_CALL* noahh_unload)();
	typedef bool(NOAHH_CALL* noahh_enable)();
	typedef bool(NOAHH_CALL* noahh_disable)();
	typedef bool(NOAHH_CALL* noahh_save_data)(const char*);
	typedef bool(NOAHH_CALL* noahh_load_data)(const char*);
	typedef void(NOAHH_CALL* noahh_setting_updated)(const char*, Setting*);
}