...
namespace noahh {
	 ...
	 * from noahh will automatically create all 
	...
	class Interface {
	protected:
		static NOAHH_DLL Interface* create();
	...
	public:
         ...
         */
        NOAHH_DLL void logInfo(std::string const& info, Severity severity);

        NOAHH_DLL void scheduleOnLoad(ScheduledFunction function);
...
inline const char* operator"" _spr(const char* str, size_t) {
    return noahh::Mod::get()->expandSpriteName(str);
}