...
namespace noahh {
    ...
    NOAHH_DLL std::string format_as(cocos2d::CCObject const*);
    NOAHH_DLL std::string format_as(cocos2d::CCArray*);
    NOAHH_DLL std::string format_as(cocos2d::CCNode*);
    class Mod;
    NOAHH_DLL std::string format_as(Mod*);
}

template <typename T, typename E>
struct fmt::formatter<noahh::Result<T, E>> {
    template <typename ParseContext>
    constexpr auto parse(ParseContext& ctx) noexcept { return ctx.begin(); }

    template <typename FormatContext>
    auto format(noahh::Result<T, E> const& result, FormatContext& ctx) const noexcept {
        std::string out;
        ...
        auto formatValue = [&](auto&& value) {
            ...
            if constexpr (std::is_same_v<V, std::string>) {
                ...
                // nothing
            } else if constexpr (requires { noahh::format_as(value); }) {
                fmt::format_to(std::back_inserter(out), "{}", noahh::format_as(value));
...
namespace noahh::log::impl {
    ...
    template <class T>
    concept IsWrappedCocos = std::is_pointer_v<std::decay_t<T>> && requires(T ptr) { noahh::format_as(ptr); };

    template <class T>
    inline decltype(auto) wrapCocosObj(T&& value) {
        if constexpr (IsWrappedCocos<T>) {
            return noahh::format_as(value);
        } else {
...
namespace noahh {
    ...
    namespace log {
        NOAHH_DLL std::string generateLogName();

        NOAHH_DLL void vlogImpl(Severity, Mod*, fmt::string_view format, fmt::format_args args);
        ...
        NOAHH_DLL std::filesystem::path const& getCurrentLogPath();

        NOAHH_DLL void pushNest(Mod* mod);
        NOAHH_DLL void popNest(Mod* mod);

        /// Blocks the current thread until all logs are printed to console/file, recommended only for debugging purposes
        NOAHH_DLL void flush();
        ...
        private:
            ...
            friend NOAHH_DLL std::shared_ptr<Nest> saveNest();
            friend NOAHH_DLL void loadNest(std::shared_ptr<Nest> const& nest);
        public:
            explicit Nest(std::shared_ptr<Nest::Impl> impl);
        };

        [[nodiscard]] NOAHH_DLL std::shared_ptr<Nest> saveNest();
        NOAHH_DLL void loadNest(std::shared_ptr<Nest> const& nest);

        class Log;

        // Borrowed log
        struct NOAHH_DLL BorrowedLog final {
        private:
            friend class Logger;
            friend struct Log;
...
template <size_t N>
inline void noahh::log::BorrowedLog::formatTo(utils::StringBuffer<N>& buf, bool millis) const {
