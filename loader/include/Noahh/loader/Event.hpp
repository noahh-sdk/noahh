...
// namespace noahh::console {
//     void log(ZStringView msg, Severity severity);
// }

namespace noahh::comm {
    template <class T>
    struct RefOrVoid {
    ...
    public:
        ...
        ReceiverHandle addReceiver(Callable receiver, int priority = 0) noexcept {
            ...
            if (m_sending > 0) {
                // noahh::console::log(fmt::format("Added handler with id {} to toAdd", handle), Severity::Debug);
            ...
            for (auto it = m_receivers.begin(); it != m_receivers.end(); ++it) {
                if (priority < it->m_priority) {
                    // noahh::console::log(fmt::format("Added handler with id {} to receivers", handle), Severity::Debug);
        ...
        size_t removeReceiver(ReceiverHandle handle) noexcept {
            ...
            for (int i = 0; i < size; ++i) {
                if (m_receivers[i].m_handle == handle) {
                    if (m_sending > 0) {
                        // noahh::console::log(fmt::format("Added handler with id {} to toRemove", handle), Severity::Debug);
                        m_toRemove.push_back(m_receivers.begin() + i);
                    } else {
                        // noahh::console::log(fmt::format("Removed handler with id {} from receivers", handle), Severity::Debug);
        ...
        bool send(Args&&... value) noexcept(std::is_nothrow_invocable_v<Callable, Args...>) {
            ...
            for (auto& callable : m_receivers) {
                if (std::find_if(m_toRemove.begin(), m_toRemove.end(), [&callable](auto& it) {
                    ...
                    // noahh::console::log(fmt::format("Skipping handler with id {} because it is in toRemove", callable.m_handle), Severity::Debug);
                    continue;
                }
            ...
            if (m_sending == 0) {
                // noahh::console::log(fmt::format("Flushing {} handlers from toRemove", m_toRemove.size()), Severity::Debug);
                ...
                // noahh::console::log(fmt::format("Flushing {} handlers from toAdd", m_toAdd.size()), Severity::Debug);
    ...
    class QueuedPort : protected Port<Callable, ThreadSafe, Container> {
        using VectorType = std::vector<noahh::CopyableFunction<void()>>;
    ...
    static_assert(PortTemplateFor<PortWrapper<Port, true, PortCallableCopy>::type, noahh::CopyableFunction<void()>>, "Port type is not a valid port");
    ...
    requires PortTemplateFor<PortTemplate, noahh::CopyableFunction<bool(PArgs...)>>
    ...
    protected:
        PortTemplate<noahh::CopyableFunction<bool(PArgs...)>> m_port;

    public:
        using EventCenterType = typename PortTemplate<noahh::CopyableFunction<bool(PArgs...)>>::EventCenterType;
        ...
        template <class... Args>
        bool send(Args&&... args) noexcept(std::is_nothrow_invocable_v<noahh::CopyableFunction<bool(PArgs...)>, Args...>) {
            return m_port.send(std::forward<Args>(args)...);
        }

        ReceiverHandle addReceiver(noahh::CopyableFunction<bool(PArgs...)> rec, int priority = 0) noexcept {
    ...
    protected:
        ...
        static OpaquePortBase* migratePort(OpaquePortBase* port) {
            if (!noahh::cast::typeinfo_cast<OpaqueEventV2Type*>(port)) {
        ...
        bool operator==(BaseFilter const& other) const noexcept override {
            // noahh::console::log(fmt::format("Comparing BasicEvent filters {}, {}", (void*)this, (void*)&other), Severity::Debug);
            // noahh::console::log(fmt::format("Self type: {}", cast::getRuntimeTypeName(this)), Severity::Debug);
            // noahh::console::log(fmt::format("Other type: {}", cast::getRuntimeTypeName(&other)), Severity::Debug);
            auto* o = noahh::cast::typeinfo_cast<Self const*>(&other);
            if (!o) return false;

            auto ret = m_filter == o->m_filter;

            // noahh::console::log(fmt::format("Comparison result: {}", ret), Severity::Debug);
            return ret;
        }
        ...
        ListenerHandle addReceiver(noahh::CopyableFunction<PReturn(PArgs...)> rec, int priority = 0) const noexcept;
        ...
        static void removeReceiverStatic(BaseFilter const* filter, ReceiverHandle handle) noexcept {
            ...
            // noahh::console::log(fmt::format("Static removing receiver from BasicEvent {}, {}", (void*)self, typeid(Marker).name()), Severity::Debug);
            if (self) {
                // noahh::console::log(fmt::format("afssfd from BasicEvent {}, {}", (void*)self, typeid(Marker).name()), Severity::Debug);
                self->removeReceiver(handle);
            }
        }

        BasicEvent(CloneMarker, std::tuple<FArgs...> const& value) noexcept : m_filter(value) {
            // noahh::console::log(fmt::format("Cloning BasicEvent {}, {}", (void*)this, typeid(Marker).name()), Severity::Debug);
        }

    public:
        BasicEvent(FArgs... value) noexcept : m_filter(std::move(value)...) {
            // noahh::console::log(fmt::format("Creating BasicEvent {}, {}", (void*)this, typeid(Marker).name()), Severity::Debug);
        }
        ~BasicEvent() noexcept override {
            // noahh::console::log(fmt::format("Destroying BasicEvent {}, {}", (void*)this, typeid(Marker).name()), Severity::Debug);
        }

        bool send(PArgs... args) noexcept(std::is_nothrow_invocable_v<noahh::CopyableFunction<PReturn(PArgs...)>, PArgs...>);
    ...
    class NOAHH_DLL EventCenterThreadLocal {
    private:
        class Impl;
    ...
    public:
        ...
        using SendFuncType = noahh::Function<bool(OpaquePortBase*)>;
        using AddFuncType = noahh::Function<ReceiverHandle(OpaquePortBase*)>;
        using SizeFuncType = noahh::Function<size_t(OpaquePortBase*)>;
        using RemoveFuncType = noahh::Function<size_t(OpaquePortBase*)>;
        using MigrateFuncType = noahh::Function<OpaquePortBase*(OpaquePortBase*)>;
    ...
    class NOAHH_DLL EventCenterGlobal {
    private:
        class Impl;
        std::unique_ptr<Impl> m_impl;
    ...
    public:
        NOAHH_DLL static EventCenter* get();
        ...
        bool send(BaseFilter const* filter, Callable func, Callable2 migratePort) noexcept(std::is_nothrow_invocable_v<Callable, OpaquePortBase*>) {
            // noahh::console::log(fmt::format("EventCenter sending event for filter {}, {}", (void*)filter, cast::getRuntimeTypeName(filter)), Severity::Debug);
            auto p = m_ports.load();
        ...
        ListenerHandle addReceiver(BaseFilter const* filter, Callable func, Callable2 migratePort) noexcept {
            // noahh::console::log(fmt::format("EventCenter adding receiver for filter {}, {}", (void*)filter, cast::getRuntimeTypeName(filter)), Severity::Debug);
            auto p = m_ports.load();
            auto it = p->find(filter);
            if (it != p->end()) {
                if (auto newFilter = std::invoke(migratePort, it->second.get())) {
                ...
                // noahh::console::log(fmt::format("Cloned filter for adding receiver {}, {}", (void*)filter2, cast::getRuntimeTypeName(filter2)), Severity::Debug);
        ...
        size_t removeReceiver(BaseFilter const* filter, Callable func, Callable2 migratePort) noexcept {
            // noahh::console::log(fmt::format("EventCenter removing receiver for filter {}, {}", (void*)filter, cast::getRuntimeTypeName(filter)), Severity::Debug);
            auto p = m_ports.load();
            auto it = p->find(filter);
            if (it != p->end()) {
                if (auto newFilter = std::invoke(migratePort, it->second.get())) {
                ...
                if (size == 0) {
                    // noahh::console::log(fmt::format("Removing port for filter type {}", cast::getRuntimeTypeName(filter)), Severity::Debug);
                    m_ports.rcu([&](auto const& ptr) {
                        auto newPorts = asp::make_shared<MapType>(*ptr.get());
    ...
    bool BasicEvent<Marker, PortTemplate, PReturn(PArgs...), FArgs...>::send(PArgs... args) noexcept(std::is_nothrow_invocable_v<noahh::CopyableFunction<PReturn(PArgs...)>, PArgs...>) {
    ...
    ListenerHandle BasicEvent<Marker, PortTemplate, PReturn(PArgs...), FArgs...>::addReceiver(noahh::CopyableFunction<PReturn(PArgs...)> rec, int priority) const noexcept {
