...
namespace noahh {
    ...
    class NOAHH_DLL NodeIDs final {
    public:
        template<class T>
        using Provider = void(NOAHH_CALL*)(T*);
    ...
    public:
        ...
        void registerProvider(void(NOAHH_CALL* fun)(T*)) {
            m_providers.insert({
                T::CLASS_NAME,
    ...
    void NOAHH_CALL noahhInternalProvideIDsFor(For* cls) {
...
#define $register_ids(Layer_) \
    struct NOAHH_CONCAT(ProvideIDsFor, Layer_) : public Layer_ {\
        void provide();\
    };\
	$execute {\
		NodeIDs::get()->registerProvider(\
            &noahhInternalProvideIDsFor<NOAHH_CONCAT(ProvideIDsFor, Layer_)>\
        );\
	};\
    void NOAHH_CONCAT(ProvideIDsFor, Layer_)::provide()
