...
namespace noahh::cocos {
     ...
     */
    NOAHH_DLL cocos2d::CCRect calculateNodeCoverage(std::vector<cocos2d::CCNode*> const& nodes);
    /**
     * Get bounds for a set of nodes. Based on content 
     * size
     * @param nodes Nodes to calculate coverage of
     ...
     * }
     */
    NOAHH_DLL bool fileExistsInSearchPaths(const char* filename);


    template <typename T>
    struct NOAHH_DLL CCArrayIterator {
    public:
     ...
     * the callback as a member to a class. Use the NOAHH_MENU_SELECTOR class 
    ...
    #define NOAHH_MENU_SELECTOR(senderArg, ...) \
