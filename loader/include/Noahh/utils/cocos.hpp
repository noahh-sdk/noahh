...
struct matjson::Serialize<cocos2d::ccColor3B> {
    static noahh::Result<cocos2d::ccColor3B> NOAHH_DLL fromJson(Value const& value);
    static Value NOAHH_DLL toJson(cocos2d::ccColor3B const& value);
};

template <>
struct matjson::Serialize<cocos2d::ccColor4B> {
    static noahh::Result<cocos2d::ccColor4B> NOAHH_DLL fromJson(Value const& value);
    static Value NOAHH_DLL toJson(cocos2d::ccColor4B const& value);
};

namespace noahh::cocos {
...
template <typename T>
struct ::noahh::CCArrayExtCheck<T, void> {
    using type = cocos::CCArrayExt<T, true>;
};

template <typename K, typename V>
struct ::noahh::CCDictionaryExtCheck<K, V, void> {
...
namespace noahh {
    ...
    class NOAHH_DLL WeakRefController final {
    private:
    ...
    class NOAHH_DLL WeakRefPool final {
...
namespace noahh::cocos {
     ...
     */
    NOAHH_DLL cocos2d::CCRect calculateNodeCoverage(std::span<cocos2d::CCNode*> nodes);
    /**
     * Get bounds for a set of nodes. Based on content
     * size
     * @param nodes Nodes to calculate coverage of
     ...
     * will be <= 0 and size will be >= 0
     */
    NOAHH_DLL cocos2d::CCRect calculateNodeCoverage(cocos2d::CCArray* nodes);
    /**
     * Get bounds for a set of nodes. Based on content
     * size
     ...
     */
    NOAHH_DLL cocos2d::CCRect calculateChildCoverage(cocos2d::CCNode* parent);
     ...
     */
    NOAHH_DLL cocos2d::CCScene* switchToScene(cocos2d::CCLayer* layer);

    using CreateLayerFunc = noahh::Function<cocos2d::CCLayer*()>;
     ...
     */
    NOAHH_DLL void reloadTextures(CreateLayerFunc returnTo = nullptr);
     ...
     */
    NOAHH_DLL void limitNodeSize(cocos2d::CCNode* node, cocos2d::CCSize const& size, float def, float min);
     ...
     */
    NOAHH_DLL void limitNodeWidth(cocos2d::CCNode* node, float width, float def, float min);
     ...
     * @param min Minimum scale
     */
    NOAHH_DLL void limitNodeHeight(cocos2d::CCNode* node, float height, float def, float min);
     ...
     */
    NOAHH_DLL bool nodeIsVisible(cocos2d::CCNode* node);
     ...
     */
    NOAHH_DLL cocos2d::CCNode* getChildByTagRecursive(cocos2d::CCNode* node, int tag);
     ...
     * name
     */
    NOAHH_DLL bool isSpriteFrameName(cocos2d::CCNode* node, const char* name);
     ...
     */
    NOAHH_DLL cocos2d::CCNode* getChildBySpriteFrameName(cocos2d::CCNode* parent, const char* name);
     ...
     */
    NOAHH_DLL bool isSpriteName(cocos2d::CCNode* node, const char* name);
     ...
     * nullptr if there is none
     */
    NOAHH_DLL cocos2d::CCNode* getChildBySpriteName(cocos2d::CCNode* parent, const char* name);
     ...
     */
    NOAHH_DLL std::string_view getObjectName(cocos2d::CCObject const* obj);
     ...
     * }
     */
    NOAHH_DLL bool fileExistsInSearchPaths(char const* filename);
     ...
     */
    NOAHH_DLL Result<cocos2d::ccColor3B> cc3bFromHexString(std::string_view hexValue, bool permissive = false);
    /**
     ...
     * indicating the failure reason
     */
    NOAHH_DLL Result<cocos2d::ccColor4B> cc4bFromHexString(std::string_view hexValue, bool requireAlpha = false, bool permissive = false);
     ...
     * @returns Hex string
     */
    NOAHH_DLL std::string cc3bToHexString(cocos2d::ccColor3B const& color);
     ...
     * @returns Hex string
     */
    NOAHH_DLL std::string cc4bToHexString(cocos2d::ccColor4B const& color);
     ...
     */
    NOAHH_DLL cocos2d::CCPoint getMousePos();
     ...
     */
    NOAHH_DLL cocos2d::CCSize getLabelSize(std::u16string_view text, const char* font, int kerning = 0);
     ...
     * @returns The size of the label
     */
    NOAHH_DLL cocos2d::CCSize getLabelSize(std::string_view text, const char* font, int kerning = 0);
}
...
namespace std {
    ...
    template <class T>
    struct hash<noahh::Ref<T>> {
        size_t operator()(noahh::Ref<T> const& ref) const {
            return std::hash<T*>()(ref.data());
        }
    };

    template <typename T>
    struct hash<noahh::WeakRef<T>> {
        size_t operator()(noahh::WeakRef<T> const& ref) const {
            // the explicit template argument is needed here because it would otherwise cast to WeakRef and recurse
            return std::hash<std::shared_ptr<noahh::WeakRefController>>{}(ref.m_controller);
        }
    };
}
...
namespace noahh::cocos {
     ...
     * for `CCArray`.
     */
    struct NOAHH_DLL CCArrayInserter {
    public:
    ...
    private:
        ...
        public:
            noahh::Function<void(Node*)> m_callback;

            static LambdaCallback* create(noahh::Function<void(Node*)> callback) {
            ...
            bool init(noahh::Function<void(Node*)> callback) {
    ...
    public:
        ...
        static cocos2d::CCMenuItem* create(
            noahh::Function<void(cocos2d::CCMenuItem*)> callback
        ) {
        ...
        static cocos2d::CCMenuItemSprite* createSprite(
            ...
            noahh::Function<void(cocos2d::CCMenuItemSprite*)> callback
        ) {
        ...
        static CCMenuItemSpriteExtra* createSpriteExtra(
            cocos2d::CCNode* normalSprite,
            noahh::Function<void(CCMenuItemSpriteExtra*)> callback
        ) {
        ...
        static CCMenuItemToggler* createToggler(
            ...
            noahh::Function<void(CCMenuItemToggler*)> callback
        ) {
        ...
        static void assignCallback(
            ...
            noahh::Function<void(Node*)> callback
        ) {
    ...
    Type* findFirstChildRecursive(cocos2d::CCNode* gnode, noahh::FunctionRef<bool(Type*)> predicate) {
    ...
    private:
        ...
        void update(float) override {
            // Make sure any `noahh::Function`s are valid
     ...
     */
    void NOAHH_DLL handleTouchPriorityWith(cocos2d::CCNode* node, int priority, bool force = false);
     ...
     * @param force Whether it should force set without smaller-than checks
     */
    void NOAHH_DLL handleTouchPriority(cocos2d::CCNode* node, bool force = false);
}
