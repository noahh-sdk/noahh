#pragma once
#include <Noahh/cocos/robtop/keyboard_dispatcher/CCKeyboardDelegate.h>
...
namespace noahh {
    ...
    struct Keybind final {
        ...
        static NOAHH_DLL Result<Keybind> fromString(std::string_view str);
        NOAHH_DLL std::string toString() const;
        NOAHH_DLL cocos2d::CCNode* createNode() const;
...
template <>
struct std::hash<noahh::Keybind> {
    size_t operator()(noahh::Keybind const& keybind) const noexcept {
        return static_cast<size_t>(keybind.key) ^ (static_cast<size_t>(keybind.modifiers.value) << 28);
    }
};

template <>
struct matjson::Serialize<noahh::Keybind> {
    static NOAHH_DLL noahh::Result<noahh::Keybind> fromJson(matjson::Value const& value);
    static NOAHH_DLL matjson::Value toJson(noahh::Keybind const& value);
};
