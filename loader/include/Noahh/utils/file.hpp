...
struct matjson::Serialize<std::filesystem::path> {
    static noahh::Result<std::filesystem::path> fromJson(matjson::Value const& value) {
        NOAHH_UNWRAP_INTO(const std::string str, value.asString());
        ...
        return noahh::Ok(
            std::filesystem::path(noahh::utils::string::utf8ToWide(str)).make_preferred()
        );
#else
        return noahh::Ok(std::filesystem::path(str).make_preferred());
#endif
    }
    ...
    static matjson::Value toJson(std::filesystem::path const& value) {
        ...
        // This is not an issue until paths actually use unicode characters
        // So we do this conversion to make sure it stores the characters correctly
        return matjson::Value(noahh::utils::string::pathToString(value));
    }
};

namespace noahh::utils::file {
    NOAHH_DLL Result<std::string> readString(std::filesystem::path const& path);
    NOAHH_DLL Result<matjson::Value> readJson(std::filesystem::path const& path);
    NOAHH_DLL Result<ByteVector> readBinary(std::filesystem::path const& path);

    template <class T>
    Result<T> readFromJson(std::filesystem::path const& file) {
        NOAHH_UNWRAP_INTO(auto json, readJson(file));
        return json.as<T>();
    }
     ...
     */
    NOAHH_DLL Result<> writeString(std::filesystem::path const& path, std::string_view data);
     ...
     * @returns Result indicating success or failure
     */
    NOAHH_DLL Result<> writeStringSafe(std::filesystem::path const& path, std::string_view data);
     ...
     * @returns Result indicating success or failure
     */
    NOAHH_DLL Result<> writeBinary(std::filesystem::path const& path, ByteSpan data);
     ...
     * @returns Result indicating success or failure
     */
    NOAHH_DLL Result<> writeBinarySafe(std::filesystem::path const& path, ByteSpan data);

    template <class T>
    Result<> writeToJson(std::filesystem::path const& path, T const& data) {
        NOAHH_UNWRAP(writeString(path, matjson::Value(data).dump()));
        return Ok();
    }

    NOAHH_DLL Result<> createDirectory(std::filesystem::path const& path);
    NOAHH_DLL Result<> createDirectoryAll(std::filesystem::path const& path);
    NOAHH_DLL Result<std::vector<std::filesystem::path>> readDirectory(
        std::filesystem::path const& path, bool recursive = false
    );

    class Unzip;

    class NOAHH_DLL Zip final {
    public:
    ...
    public:
        ...
        void setProgressCallback(
            noahh::Function<void(uint32_t, uint32_t)> callback
        );
        ...
        static Result<> intoDir(
            noahh::Function<void(uint32_t, uint32_t)> progressCallback,
            Path const& from,
            Path const& to,
            bool deleteZipAfter = false
        );
     ...
     */
    NOAHH_DLL bool openFolder(std::filesystem::path const& path);
     ...
     */
    NOAHH_DLL arc::Future<PickResult> pick(PickMode mode, FilePickOptions options);
     ...
     */
    NOAHH_DLL arc::Future<PickManyResult> pickMany(FilePickOptions options);
}
