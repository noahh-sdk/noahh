...
namespace noahh::utils::coro {
    ...
    NOAHH_DLL arc::Future<void> nextFrame();
    NOAHH_DLL arc::Future<void> skipFrames(int frames);
    ...
    #define $async(...) noahh::utils::coro::spawn << [__VA_ARGS__]() -> noahh::Task<void>
    #define $try noahh::utils::coro::spawn << [&]() -> noahh::utils::coro::TryResult
};

template <typename T = void, typename E = std::string>
auto operator co_await(noahh::Result<T, E>&& res) noexcept {
    return noahh::utils::coro::ResultAwaiter<T, E> { std::move(res) };
}
template <typename T = void, typename E = std::string>
auto operator co_await(noahh::Result<T, E> const& res) noexcept {
    return noahh::utils::coro::ResultAwaiter<T, E> { res };
}

template <typename T, typename E, typename ...Args>
struct std::coroutine_traits<noahh::Result<T, E>, Args...> {
    using promise_type = noahh::utils::coro::ResultPromise<T, E>;
};

template <typename E, typename ...Args>
struct std::coroutine_traits<std::tuple<noahh::Result<void, E>>, Args...> {
    using promise_type = noahh::utils::coro::TryResultPromise<E>;
};

template <typename T, typename ...Args>
struct std::coroutine_traits<noahh::utils::coro::Generator<T>, Args...> {
    using promise_type = noahh::utils::coro::Generator<T>::promise_type;
};
