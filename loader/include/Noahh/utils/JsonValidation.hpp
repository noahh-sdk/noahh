...
namespace noahh {
    ...
    template <class T>
    using JsonValueValidator = noahh::Function<bool(T const&)>;

    struct JsonMaybeObject;
    struct JsonMaybeValue;

    class NOAHH_DLL JsonExpectedValue final {
    protected:
        class Impl;
    ...
    NOAHH_DLL JsonExpectedValue checkJson(matjson::Value json, std::string rootScopeName);
}
