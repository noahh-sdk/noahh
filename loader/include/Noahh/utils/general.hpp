...
namespace noahh {
    ...
    namespace utils {
        ...
        namespace _detail {
            NOAHH_DLL Result<long double> longDoubleFromString(std::string_view str);
            NOAHH_DLL Result<double> doubleFromString(std::string_view str);
            NOAHH_DLL Result<float> floatFromString(std::string_view str);

            NOAHH_DLL Result<uint64_t> uint64FromString(std::string_view str, int base = 10);
            NOAHH_DLL Result<int64_t> int64FromString(std::string_view str, int base = 10);
         ...
         */
        NOAHH_DLL std::string timePointAsString(std::chrono::system_clock::time_point const& tp);
         ...
         */
        NOAHH_DLL std::string timeToAgoString(std::chrono::system_clock::time_point tp, bool abbreviated = false);
        /**
         * Converts a time point to a relative string in a format like "2 days ago"
         * @param abbreviated If true, the string is abbreviated to "2d" etc.
         */
        NOAHH_DLL std::string timeToAgoString(asp::SystemTime tp, bool abbreviated = false);
        ...
        */
        NOAHH_DLL float getDisplayFactor();
         ...
         */
        NOAHH_DLL std::string getEnvironmentVariable(ZStringView name);
         ...
         */
        NOAHH_DLL std::string formatSystemError(int code);
         ...
         */
        NOAHH_DLL cocos2d::CCRect getSafeAreaRect();

        /**
         * Gets the current input timestamp in seconds.
         */
        NOAHH_DLL double getInputTimestamp();
    }
...
template<>
struct matjson::Serialize<noahh::ByteVector> {
    static Value toJson(noahh::ByteVector const& bytes) {
        return std::vector<matjson::Value>(bytes.begin(), bytes.end());
    }

    static noahh::Result<noahh::ByteVector, std::string> fromJson(Value const& value) {
        if (auto arr = value.as<std::vector<matjson::Value>>()) {
            noahh::ByteVector bytes;
            for (auto const& item : *arr) {
                if (auto byte = item.as<uint8_t>()) {
                    bytes.push_back(*byte);
                }
            }
            return noahh::Ok(bytes);
        }
        return noahh::Err("Expected a JSON array of numbers");
    }
};
...
namespace noahh::utils::clipboard {
     ...
     */
    NOAHH_DLL bool write(ZStringView data);
...
namespace noahh::utils::game {
     ...
     * @param saveData Whether to save the game data
     */
    NOAHH_DLL void restart(bool saveData /* = true */);
     ...
     */
    NOAHH_DLL void launchLoaderUninstaller(bool deleteSaveData);
}
...
namespace noahh::utils::thread {
     ...
     */
    NOAHH_DLL std::string getDefaultName();
     ...
     */
    NOAHH_DLL void setName(std::string name);
}
