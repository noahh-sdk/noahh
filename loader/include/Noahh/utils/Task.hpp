...
namespace noahh {
     ...
     * to using the Noahh events system; tasks may have multiple listeners, and
    ...
    public:
        ...
        private:
            ...
            friend struct noahh_internal::TaskPromiseBase;

            template <class, class>
            friend struct noahh_internal::TaskAwaiter;
        ...
        using Value            = Type;
        using Progress         = P;
        using PostResult       = noahh::CopyableFunction<void(Result&&)>;
        using PostProgress     = noahh::CopyableFunction<void(P)>;
        using HasBeenCancelled = noahh::CopyableFunction<bool()>;
        using Run              = noahh::Function<Result(PostProgress, HasBeenCancelled)>;
        using RunWithCallback  = noahh::Function<void(PostResult, PostProgress, HasBeenCancelled)>;
...
template <class T, class P>
auto operator co_await(noahh::Task<T, P> task) {
    return noahh::noahh_internal::TaskAwaiter<T, P>{task};
}

template <class T, class P, class... Args>
struct std::coroutine_traits<noahh::Task<T, P>, Args...> {
    using promise_type = noahh::noahh_internal::TaskPromise<T, P>;
};
