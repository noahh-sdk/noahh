...
namespace noahh {
    class NOAHH_DLL VMTHookManager final {
    private:
        class Impl;
    ...
    #if defined(NOAHH_IS_WINDOWS32) || defined(NOAHH_IS_WINDOWS64)
        template <auto UUID>
    ...
    #elif defined(NOAHH_IS_ARM_MAC) || defined(NOAHH_IS_ANDROID64) || defined(NOAHH_IS_IOS)
        template <auto UUID>
        static void __attribute__((naked)) emptyFunction() {
            __asm__ volatile(
                "0:\n\t"                 
        ...
        Result<std::optional<Hook*>> addHook(
            ...
            if constexpr (Function != nullptr) {
                ...
                auto const handlerMetadata = tulip::hook::HandlerMetadata{
                    .m_convention = noahh::hook::createConvention(convention),
                ...
                NOAHH_UNWRAP_INTO(auto hook, this->addHookInternal(
                ...
                if (hook.has_value()) {
                    NOAHH_UNWRAP_INTO(auto hook, Mod::get()->claimHook(*hook));
                    return Ok(hook);
                }
            }
