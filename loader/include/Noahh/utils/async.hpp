...
arc::TaskHandle<void> spawn(Fut future, Callback cb) {
    return runtime().spawn([](Fut future, Callback cb) mutable -> arc::Future<> {
        if constexpr (Void) {
            ...
            noahh::queueInMainThread([cb = std::move(cb)] mutable {
                cb();
            });
        } else {
...
private:
    ...
    void spawnInner(std::string name, F&& future, Callback cb) {
        ...
        if constexpr (std::is_void_v<Ret>) {
            state->m_handle = noahh::async::spawn(std::forward<F>(future), [state, cb = std::move(cb)] mutable {
                if (state->isCancelled()) return;
                state->complete();
                cb();
            });
        } else {
            state->m_handle = noahh::async::spawn(std::forward<F>(future), [state, cb = std::move(cb)](Ret val) mutable {
                if (state->isCancelled()) return;
                state->complete();
                cb(std::move(val));
            });
