    ...
    struct SpawnPollable : arc::Pollable<SpawnPollable, void> {
        ...
        bool poll(arc::Context& cx) {
            ...
            if constexpr (Void) {
                ...
                // scary things
                noahh::queueInMainThread(std::move(*m_callback));
                m_callback.reset();
            } else {
                // more scary things
                noahh::queueInMainThread([cb = std::move(*m_callback), result = std::move(m_fut->m_vtable->template getOutput<Out>(&*m_fut))] mutable {
        ...
        ~SpawnPollable() {
            if (m_callback) {
                ...
                noahh::queueInMainThread([cb = std::move(*m_callback)] {});
            }
        }
...
struct WaitForMainAwaiter : arc::Pollable<WaitForMainAwaiter<T>, PollOut> {
    ...
    explicit WaitForMainAwaiter(F&& func) {
        ...
        noahh::queueInMainThread([state = m_state, func = std::forward<F>(func), tx = std::move(tx)] mutable {
...
private:
    ...
    void spawnInner(std::string name, F&& future, Cb&& cb) {
        ...
        if constexpr (std::is_void_v<Ret>) {
            state->m_handle = noahh::async::spawn(std::forward<F>(future), [state, cb = std::forward<Cb>(cb)] mutable {
                if (state->isCancelled()) return;
                state->complete();
                cb();
            });
        } else {
            state->m_handle = noahh::async::spawn(std::forward<F>(future), [state, cb = std::forward<Cb>(cb)](Ret val) mutable {
                if (state->isCancelled()) return;
                state->complete();
                cb(std::move(val));
            });
