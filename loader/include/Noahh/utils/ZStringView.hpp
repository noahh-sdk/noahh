...
template <>
struct matjson::Serialize<noahh::ZStringView> {
    static matjson::Value toJson(noahh::ZStringView const& value) {
        return matjson::Value(value.view());
    }
};

// inline std::string_view format_as(noahh::ZStringView zsv) {
//     return std::string_view(zsv);
// }

template <>
struct fmt::formatter<noahh::ZStringView> : formatter<string_view> {
    auto format(noahh::ZStringView const& str, fmt::format_context& ctx) const noexcept {
