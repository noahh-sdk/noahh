...
namespace noahh::utils::random {
    ...
    NOAHH_DLL Generator& _getGenerator();
    ...
    NOAHH_DLL std::string generateString(size_t length, std::string_view alphabet);
    NOAHH_DLL std::string generateHexString(size_t length);
    NOAHH_DLL std::string generateAlphanumericString(size_t length);

    /// Generates a random UUID v4 string
    NOAHH_DLL std::string generateUUID();

    NOAHH_DLL void fillBytes(void* buffer, size_t size);
}
