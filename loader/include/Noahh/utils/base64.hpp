...
namespace noahh::utils::base64 {
    ...
    NOAHH_DLL std::string encode(std::span<std::uint8_t const> data, Base64Variant var = Base64Variant::UrlWithPad);
    ...
    NOAHH_DLL std::string encode(std::string_view str, Base64Variant var = Base64Variant::UrlWithPad);
    ...
    NOAHH_DLL Result<std::vector<std::uint8_t>> decode(std::string_view str, Base64Variant var = Base64Variant::Url);

    /// Same as decode() but returns decoded data as a std::string. No validation is made if the
    /// string is "valid".
    /// @param str Base64 encoded string
    /// @return Decoded data, or an error if decoding failed
    NOAHH_DLL Result<std::string> decodeString(std::string_view str, Base64Variant var = Base64Variant::Url);
}
