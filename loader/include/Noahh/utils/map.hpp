...
namespace noahh::utils::map {
    ...
    bool contains(std::unordered_map<T, R, H> const& map, noahh::FunctionRef<bool(R)> containFunc) {
    ...
    R select(std::unordered_map<T, R, H> const& map, noahh::FunctionRef<bool(R)> selectFunc) {
        for (auto const& [_, r] : map) {
    ...
    std::vector<R> selectAll(
        std::unordered_map<T, R, H> const& map, noahh::FunctionRef<bool(R)> selectFunc
    ) {
        std::vector<R> res;
        for (auto const& [_, r] : map) {
            if (selectFunc(r)) {
    ...
    std::unordered_map<T2, V2, H2> remap(
        ...
        noahh::Function<std::pair<T2, V2>(std::pair<T1, V1>)> remapFunc
    ) {
