...
#include <Noahh/loader/Loader.hpp> // another great circular dependency fix
...
namespace noahh::utils::web {
    NOAHH_DLL void openLinkInBrowser(ZStringView url);
    ...
    ///     .bodyMultipart(form)
    ///     .get(url);
    class NOAHH_DLL MultipartForm final {
    private:
        class Impl;
    ...
    class NOAHH_DLL WebResponse final {
    private:
        class Impl;
    ...
    class NOAHH_DLL WebRequest final {
    private:
        class Impl;
    ...
    public:
        ...
        WebFuture send(std::string method, std::string url, Mod* mod = noahh::getMod());
        WebFuture post(std::string url, Mod* mod = noahh::getMod());
        WebFuture get(std::string url, Mod* mod = noahh::getMod());
        WebFuture put(std::string url, Mod* mod = noahh::getMod());
        WebFuture patch(std::string url, Mod* mod = noahh::getMod());

        WebResponse sendSync(std::string method, std::string url, Mod* mod = noahh::getMod());
        WebResponse postSync(std::string url, Mod* mod = noahh::getMod());
        WebResponse getSync(std::string url, Mod* mod = noahh::getMod());
        WebResponse putSync(std::string url, Mod* mod = noahh::getMod());
        WebResponse patchSync(std::string url, Mod* mod = noahh::getMod());
         ...
         * Defaults to sending the Noahh CA bundle, found here: https://github.com/noahh-sdk/net_libs/blob/main/ca_bundle.h
         *
         * @param content
         * @return WebRequest&
         */
    ...
    struct NOAHH_DLL ARC_NODISCARD WebFuture : arc::Pollable<WebFuture, WebResponse> {
