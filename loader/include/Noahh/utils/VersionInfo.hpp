...
namespace noahh {
     ...
     */
    class NOAHH_DLL VersionInfo final {
    protected:
        size_t m_major = 1;
        size_t m_minor = 0;
        size_t m_patch = 0;
    ...
    public:
        ...
        friend NOAHH_DLL std::string format_as(VersionInfo const& version);
    };

    class NOAHH_DLL ComparableVersionInfo final {
    protected:
    ...
    public:
        ...
        std::string toString() const;
        friend NOAHH_DLL std::string format_as(ComparableVersionInfo const& version);
    };

    bool NOAHH_DLL semverCompare(VersionInfo const& current, VersionInfo const& target);
}

template <class V>
requires std::is_same_v<V, noahh::VersionInfo> || std::is_same_v<V, noahh::ComparableVersionInfo>
struct matjson::Serialize<V> {
    static noahh::Result<V, std::string> fromJson(Value const& value) {
        NOAHH_UNWRAP_INTO(auto str, value.asString());
        NOAHH_UNWRAP_INTO(auto version, V::parse(str).mapErr([](auto&& err) {
            return fmt::format("Invalid version format: {}", err);
        }));
        return noahh::Ok(version);
    }
