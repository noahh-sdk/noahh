...
#elif defined(NOAHH_IS_MACOS) || defined(NOAHH_IS_ANDROID)
	struct StringData {
		struct Internal {
			size_t m_size;
			size_t m_capacity;
...
namespace gd {
#if defined(NOAHH_IS_MACOS) || defined(NOAHH_IS_WINDOWS) || defined(NOAHH_IS_IOS)
	...
	class NOAHH_DLL string {
		noahh::stl::StringData m_data;
		friend noahh::stl::StringImpl;
	public:
		string();
		string(string const&);
		// string(string&&);
