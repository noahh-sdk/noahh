...
_GLIBCXX_BEGIN_NAMESPACE_CONTAINER
  ...
  template<typename _Tp, typename _Alloc>
    typename noahh::stl::vector<_Tp, _Alloc>::iterator
    noahh::stl::vector<_Tp, _Alloc>::
#if __cplusplus >= 201103L
  ...
  template<typename _Tp, typename _Alloc>
    noahh::stl::vector<_Tp, _Alloc>&
    noahh::stl::vector<_Tp, _Alloc>::
    operator=(const noahh::stl::vector<_Tp, _Alloc>& __x)
    {
      if (&__x != this)
	{
#if __cplusplus >= 201103L
  ...
  template<typename _Tp, typename _Alloc>
    ...
    _M_fill_assign(size_t __n, const value_type& __val)
    {
      if (__n > capacity())
	{
	  noahh::stl::vector __tmp(__n, __val, _M_get_Tp_allocator());
	  __tmp.swap(*this);
	}
      else if (__n > size())
	{
  ...
  // noahh::stl::vector<bool>
  template<typename _Alloc>
    void
    noahh::stl::vector<bool, _Alloc>::
  ...
  template<typename _Alloc>
	...
	{
	  const size_type __len = 
	    _M_check_len(__n, "noahh::stl::vector<bool>::_M_fill_insert");
	...
	if (__first != __last)
	  {
		...
		this->_M_impl._M_finish += difference_type(__n);
	      ...
	      {
		const size_type __len =
		  _M_check_len(__n, "noahh::stl::vector<bool>::_M_insert_range");
		_Bit_type * __q = this->_M_allocate(__len);
		iterator __i = _M_copy_aligned(begin(), __position,
					       iterator(__q, 0));
  ...
  template<typename _Alloc>
	...
	{
	  const size_type __len =
	    _M_check_len(size_type(1), "noahh::stl::vector<bool>::_M_insert_aux");
	  _Bit_type * __q = this->_M_allocate(__len);
	  iterator __i = _M_copy_aligned(begin(), __position,
					 iterator(__q, 0));
	  *__i++ = __x;
  ...
  template<typename _Alloc>
    typename noahh::stl::vector<bool, _Alloc>::iterator
    noahh::stl::vector<bool, _Alloc>::
    _M_erase(iterator __position)
    {
      if (__position + 1 != end())
