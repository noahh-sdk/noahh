...
namespace gd {
    ...
    using vector = noahh::stl::vector<Tp, Alloc>;

    template<typename Key, typename Tp, typename Compare = noahh::stl::less<Key>, typename Alloc = allocator<Key>>
    using map = noahh::stl::map<Key, Tp, Compare, Alloc>;

    template<typename Key, typename Compare = noahh::stl::less<Key>, typename Alloc = allocator<Key>>
    using set = noahh::stl::set<Key, Compare, Alloc>;

    template <class Key, class Tp, class Hash = noahh::stl::hash<Key>, class Pred = noahh::stl::equal_to<Key>, class Alloc = allocator<std::pair<const Key, Tp>>>
    using unordered_map = noahh::stl::unordered_map<Key, Tp, Hash, Pred, Alloc>;

    template <class Value, class Hash = noahh::stl::hash<Value>, class Pred = noahh::stl::equal_to<Value>, class Alloc = allocator<Value>>
    using unordered_set = noahh::stl::unordered_set<Value, Hash, Pred, Alloc>;
};
